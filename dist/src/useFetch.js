...
function useFetch(path, options) {
    ...
    const sendCounter = (0, react_1.useRef)(0);
    ...
    const send = (0, react_1.useCallback)(async (sendOptions) => {
        ...
        const sendId = sendCounter.current;
        ...
        try {
            ...
            response = await connection.fetch(path, { signal: abortController.signal, ...fetchOptions });
            if (!response.ok) {
            ...
            if (mergedOptions.json) {
                ...
                decodedStreamReader.closed.catch((error) => {
                    console.log("from place 1", { error });
                    if (sendId === sendCounter.current) {
                ...
                (async () => {
                    ...
                    while (!done) {
                        ...
                        if (sendId !== sendCounter.current) {
                            ...
                            decodedStreamReader.releaseLock();
                            await updateStream.cancel();
                            break;
                        }
                        if (value) {
                            responseText += value;
                    ...
                    console.log("from place 2", { error });
                    if (sendId === sendCounter.current) {
                        dispatchError(mounted, dispatch, error, response);
                    }
                });
