        ...
        try {
            ...
            if (mergedOptions.json) {
                ...
                decodedStreamReader.closed.catch((error) => {
                    console.log("place 1", { error, signal: abortContoller.signal });
                ...
                (async () => {
                    ...
                    let done = false;
                    while (!done) {
                        // if (!(mounted.current && sendId === sendCounter.current)) {
                        ...
                        console.log({ value, done, aborted: abortContoller.signal.aborted });
                        if (value) {
                            responseText += value;
                            if (!abortContoller.signal.aborted) {
                    ...
                    console.log("place 2", { error, signal: abortContoller.signal });
                    if (!abortContoller.signal.aborted) {
                        dispatchError(mounted, dispatch, error, response);
                    }
                });
