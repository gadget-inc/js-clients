...
class GadgetConnection {
    ...
    async waitForOpenedConnection(options) {
        ...
        return await new Promise((resolve, reject) => {
            ...
            const retryOnClose = (closeEvent) => {
                let message = "unknown close event";
                if (closeEvent instanceof CloseEvent) {
                    if (closeEvent.code == graphql_ws_1.CloseCode.ConnectionAcknowledgementTimeout && attempts > 0) {
                        attempts -= 1;
                        void subscriptionClient.dispose();
                    ...
                    message = `CloseEvent: ${closeEvent.reason}`;
                }
