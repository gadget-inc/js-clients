...
const internalBulkCreateMutation = (apiIdentifier, pluralApiIdentifier) => {
    ...
    mutation InternalBulkCreate${capitalizedPluralApiIdentifier}($records: [Internal${capitalizedApiIdentifier}Input]!, $select: [String!]) {
      ${internalHydrationPlan(apiIdentifier)}
      internal {
        bulkCreate${capitalizedPluralApiIdentifier}(${pluralApiIdentifier}: $records, select: $select) {
          success
          errors {
            ... InternalErrorsDetails
          }
...
class InternalModelManager {
    ...
    async create(record, options) {
        return await this.transaction(async (transaction) => {
            const response = await transaction.client
                ...
                select: formatInternalSelectVariable(options === null || options === void 0 ? void 0 : options.select),
            })
                .toPromise();
     ...
     */
    async update(id, record, options) {
