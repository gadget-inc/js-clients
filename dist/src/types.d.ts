 ...
 * ```
 */
export declare type SelectElement<Schema, Selection extends FieldSelection | null | undefined> = Selection extends null | undefined ? never : Schema extends (infer T)[] ? SelectElement<T, Selection>[] : Schema extends null ? SelectElement<Exclude<Schema, null>, Selection> | null : FilterNever<{
    [Key in keyof Selection & keyof Schema]: Selection[Key] extends true ? Schema[Key] : Selection[Key] extends FieldSelection ? SelectElement<Schema[Key], Selection[Key]> : never;
}>;
/**
 * Extract a subset of a schema given a selection
 *
 ...
 * ```
 */
export declare type Select<Schema extends Record<string, any>, Selection extends FieldSelection> = FilterNever<{
    [Key in keyof Selection & keyof Schema]: Selection[Key] extends true ? Schema[Key] : Selection[Key] extends FieldSelection ? SelectElement<Schema[Key], Selection[Key]> : never;
}>;
