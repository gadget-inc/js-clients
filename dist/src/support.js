...
class InvalidRecordError extends Error {
    constructor(message, validationErrors, modelApiIdentifier, record) {
        super(message !== null && message !== void 0 ? message : `Record is invalid and can't be saved. ${validationErrors.length} ${validationErrors.length == 1 ? "error needs" : "errors need"} to be corrected.`);
        Object.defineProperty(this, "code", {
            enumerable: true,
            configurable: true,
            writable: true,
...
const gadgetErrorFor = (error) => {
    if (error.code == "GGT_INVALID_RECORD") {
        return new InvalidRecordError(error.message, error.validationErrors);
    }
