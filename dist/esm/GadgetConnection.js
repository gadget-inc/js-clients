...
const liveQueryExchange = ({ forward }) => {
    ...
    return operations$ => {
        ...
        const live = pipe(operations$, filter(op => op.query.definitions.some(isLiveQueryOperationDefinitionNode)), filter(op => !executed.has(op.key) || op.kind !== "query"), // We will forward on everything that hasn't been executed and is not a query
        tap((op) => {
            if (op.kind == "query") {
                executed.add(op.key);
            }
