...
import { GadgetUnexpectedCloseError, GadgetWebsocketConnectionTimeoutError, getCurrentSpan, isCloseEvent, storageAvailable, traceFunction, } from "./support.js";
    ...
    newBaseClient() {
        ...
        subscriptionExchange({
            isSubscriptionOperation: (request) => {
                return request.query.definitions.some((definition) => isLiveQueryOperationDefinitionNode(definition, request.variables));
            },
            forwardSubscription: (request) => {
                return {
                    subscribe: (sink) => {
                        const input = { ...request, query: request.query || "" };
                        return {
                            unsubscribe: applyAsyncIterableIteratorToSink(applyLiveQueryJSONDiffPatch(makeAsyncIterableIteratorFromSink((sink) => this.baseSubscriptionClient.subscribe(input, sink))), sink),
                        };
                    },
                };
            },
