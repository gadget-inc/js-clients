...
import { GadgetTooManyRequestsError, GadgetUnexpectedCloseError, GadgetWebsocketConnectionTimeoutError, isCloseEvent, storageAvailable, } from "./support.js";
...
const cacheControlExchange = ({ forward }) => {
    const executed = new Set();
    return operations$ => {
        const cacheable = pipe(operations$, filter(op => !op.query.definitions.some(isLiveQueryOperationDefinitionNode)));
        const uncacheable = pipe(operations$, filter(op => op.query.definitions.some(isLiveQueryOperationDefinitionNode)), map((op) => {
            if (executed.has(op.key)) {
                console.log("changing network policy");
                op.context.requestPolicy = "cache-only"; // Update the network policy so it only reads from the cache
            }
            else {
                executed.add(op.key);
            }
        ...
        return forward(merge([cacheable, uncacheable]));
    };
};
/**
...
export class GadgetConnection {
    ...
    newBaseClient() {
        ...
        if (typeof window != "undefined") {
            exchanges.push(cacheControlExchange);
        ...
        client.reexecuteOperation = (operation) => {
            console.log("reexecute", operation);
