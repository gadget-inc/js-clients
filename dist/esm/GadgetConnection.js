...
import { otelExchange } from "./otelExchange.js";
import { rateLimitExchange } from "./rateLimitExchange.js";
import { GadgetUnexpectedCloseError, GadgetWebsocketConnectionTimeoutError, TokenBucket, getCurrentSpan, isCloseEvent, onSpanError, storageAvailable, traceFunction, withSpan, } from "./support.js";
...
export class GadgetConnection {
    constructor(options) {
        ...
        Object.defineProperty(this, "transaction", {
            ...
            configurable: true,
            writable: true,
            value: traceFunction("api-client.transaction", async (optionsOrRun, maybeRun) => {
                var _a;
                let run;
                let options;
                if (maybeRun) {
                ...
                (_a = getCurrentSpan()) === null || _a === void 0 ? void 0 : _a.setAttributes({ applicationId: this.options.applicationId, environmentName: this.environment });
                ...
                try {
                    ...
                    const client = new Client({
                        ...
                        exchanges: [
                            otelExchange,
                            rateLimitExchange(this.operationBucket),
                            subscriptionExchange({
                                forwardSubscription(request) {
                                    const input = { ...request, query: request.query || "" };
                                    return {
                                        subscribe: (sink) => {
                                            return withSpan("api-client.ws", (span) => {
                                                const sinkWrapper = {
                                                    next: (value) => {
                                                        sink.next(value);
                                                    },
        ...
        Object.defineProperty(this, "fetch", {
            ...
            configurable: true,
            writable: true,
            value: traceFunction("api-client.fetch", async (input, init = {}) => {
                var _a;
        ...
        this.operationBucket = new TokenBucket({
    ...
    newBaseClient() {
        const exchanges = [otelExchange, urlParamExchange];
        // apply urql's default caching behaviour when client side (but skip it server side)
        if (typeof window != "undefined") {
            exchanges.push(cacheExchange);
        }
        exchanges.push(rateLimitExchange(this.operationBucket), fetchExchange, subscriptionExchange({
            forwardSubscription: (request) => {
                return {
                    subscribe: (sink) => {
                        const input = { ...request, query: request.query || "" };
