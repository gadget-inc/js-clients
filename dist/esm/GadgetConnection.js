...
const liveQueryExchange = ({ forward }) => {
    ...
    return operations$ => {
        const notLive = pipe(operations$, filter(op => op.kind !== "query" || !op.query.definitions.some(isLiveQueryOperationDefinitionNode)));
        const live = pipe(operations$, filter(op => op.kind == "query" && op.query.definitions.some(isLiveQueryOperationDefinitionNode)), filter(op => !executed.has(op.key)), tap((op) => {
            executed.add(op.key);
        }));
