...
const cacheControlExchange = ({ forward }) => {
    return operations$ => {
        ...
        const uncacheable = pipe(operations$, filter(op => op.query.definitions.some(isLiveQueryOperationDefinitionNode)), tap((op) => op.context.requestPolicy = "network-only") // Update the network policy so they are never cached
        );
