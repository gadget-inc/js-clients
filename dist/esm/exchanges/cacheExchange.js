...
import { formatDocument, makeOperation, makeResult, } from "@urql/core";
...
const shouldSkip = ({ kind, query }) => (kind !== "mutation" && kind !== "query") || query.definitions.some(isLiveQueryOperationDefinitionNode);
/** Adds unique typenames to query (for invalidating cache entries) */
export const mapTypeNames = (operation) => {
 ...
 */
export const cacheExchange = ({ forward, client, dispatchDebug }) => {
    ...
    return (ops$) => {
        const cachedOps$ = pipe(ops$, filter((op) => !shouldSkip(op) && isOperationCached(op)), map((operation) => {
        ...
        const forwardedOps$ = pipe(merge([
            pipe(ops$, filter((op) => !shouldSkip(op) && !isOperationCached(op)), map(mapTypeNames)),
            pipe(ops$, filter((op) => shouldSkip(op))),
        ]), map((op) => addMetadata(op, { cacheOutcome: "miss" })), filter((op) => op.kind !== "query" || op.context.requestPolicy !== "cache-only"), forward, tap((response) => {
 ...
 * @internal
 */
export const reexecuteOperation = (client, operation) => {
