...
export const useFormFields = (metadata, options) => {
    return useMemo(() => {
        ...
        const includedObjectFields = objectFields.flatMap((objectField) => filterAutoFormFieldList(objectField.configuration.fields, options).map((innerField) => ({
...
export const useAutoForm = (props) => {
    ...
    const { submit, error: formError, reset, formState: { isSubmitSuccessful, isLoading, isReady, isSubmitting, touchedFields }, originalFormMethods, } = useActionForm(action, {
