...
export const makeShadcnAutoTableBulkActionSelector = (elements) => {
    ...
    function NonPromotedActionsActionSelector(props) {
        ...
        return (React.createElement(Popover, { open: open, onOpenChange: setOpen },
            React.createElement(PopoverTrigger, { asChild: true },
                React.createElement(Button, { variant: "outline", role: "combobox", "aria-expanded": open, className: `w-[200px] justify-between ${open ? "ring-ring/50 ring-3" : ""}`, "aria-label": "More actions", onClick: () => setOpen(!open) },
                    "Actions...",
            ...
            React.createElement(PopoverContent, { className: "w-[200px] p-0" },
                React.createElement(Command, null,
                    React.createElement(CommandList, null,
                        React.createElement(CommandGroup, null, nonPromotedActions.map((action, i) => (React.createElement(CommandItem, { key: i, className: "bg-background", onSelect: () => {
    ...
    function PromotedActionsActionSelector(props) {
        ...
        const selectedRows = rows.filter((row) => selection.recordIds.includes(row.id));
        return (React.createElement(React.Fragment, null, promotedActions.map((action) => (React.createElement(Button, { variant: "outline", key: action.humanizedName, onClick: () => {
                getBulkActionOptionCallback({
                    option: action,
                    selectedRows,
                    clearSelection: selection.clearAll,
    ...
    function ShadcnAutoTableBulkActionSelector(props) {
        ...
        return (React.createElement(React.Fragment, null,
            React.createElement(PromotedActionsActionSelector, { promotedActions: promotedActions, selection: selection, rows: rows, rawRecords: rawRecords }),
            React.createElement(NonPromotedActionsActionSelector, { nonPromotedActions: nonPromotedActions, selection: selection, rows: rows, rawRecords: rawRecords })));
    }
