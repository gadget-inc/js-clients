...
import { default as React, Suspense, useCallback, useState } from "react";
        ...
        return (React.createElement(Popover, { open: isOpen, onOpenChange: setIsOpen },
            React.createElement(PopoverTrigger, { asChild: true },
                React.createElement("div", { className: "flex flex-col gap-2" },
                    ...
                    React.createElement(Button, { id: props.id ? `${props.id}-date` : undefined, onClick: () => setIsOpen(!isOpen), variant: "outline", type: "button", className: `w-full justify-start text-left font-normal ${!localTime ? "text-muted-foreground" : ""} ${isOpen ? "ring-ring/50 ring-3" : ""}` },
                        React.createElement(CalendarIcon, { className: "mr-2 h-4 w-4" }),
                        localTime ? (formatDate(localTime, config.includeTime, true)) : (React.createElement("span", { className: "opacity-50" }, config.includeTime ? "YYYY-MM-DD HH:mm" : "YYYY-MM-DD")),
            ...
            React.createElement(PopoverContent, { className: "w-auto p-0" },
                React.createElement("div", { className: "flex flex-row flex-nowrap" },
                    React.createElement("div", { className: "relative bg-background" },
                        React.createElement(Calendar, { mode: "single", defaultMonth: localTime, selected: localTime, onSelect: handleDateSelect, autoFocus: true })),
                    config.includeTime && (React.createElement("div", { className: "flex flex-col p-4 bg-background border-l" },
