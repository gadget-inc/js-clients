...
export const makeShadcnListMessages = ({ CommandEmpty, CommandItem, Checkbox, Label, }) => {
    ...
    function ShadcnSelectableOption(props) {
        ...
        return (React.createElement(CommandItem, { key: id, id: id, value: `${label}`, selected: selected, className: className, onMouseDown: (e) => {
                e.preventDefault();
                e.stopPropagation();
            }, onSelect: () => {
                onSelect?.(id);
            ...
            React.createElement(Label, { htmlFor: id, className: "flex-1 ml-2" }, labelElement))) : (labelElement)));
    }
    return {
        NoRecordsMessage,
