...
export const makeShadcnAutoHasOneInput = ({ Badge, Button, Command, CommandItem, CommandInput, Label, CommandList, CommandEmpty, CommandGroup, Checkbox, }) => {
    ...
    function ShadcnAutoHasOneInput(props) {
        ...
        const selectedRecordTag = selectedOption ? (React.createElement(SelectedItemBadge, { key: `selectedRecordTag_${selectedOption.id}`, id: `selectedRecordTag_${selectedOption.id}`, content: React.createElement("p", { id: `${selectedOption.id}_${selectedOption.primary}` }, selectedOption.primary ?? `id: ${selectedOption.id}`), onRemoveRecord: () => selectedRecord && onRemoveRecord(selectedRecord) })) : null;
