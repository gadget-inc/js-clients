...
export const makeShadcnAutoHasManyThroughForm = ({ Badge, Button, Label, Command, CommandInput, CommandItem, CommandList, CommandEmpty, CommandGroup, Checkbox, Popover, PopoverContent, PopoverTrigger, }) => {
    ...
    const SiblingOption = (props) => {
        ...
        return (React.createElement(CommandItem, { onSelect: () => {
            ...
            React.createElement("div", { className: "flex justify-between items-center w-full cursor-pointer" },
                React.createElement("div", { className: "flex flex-row items-center gap-2 cursor-pointer" },
                    renderOptionLabel(option.primary, "primary", true),
                    option.secondary && renderOptionLabel(option.secondary, "secondary", true)),
                React.createElement(PlusIcon, { className: "w-4 h-4 shrink-0 cursor-pointer" }))));
    };
    function ShadcnAutoHasManyThroughForm(props) {
        const [open, setOpen] = useState(false);
        const { field, append, remove, joinRecords, metadata, listboxId, pathPrefix, metaDataPathPrefix, recordLabel, siblingModelName, siblingRecordsLoading, siblingRecords, siblingPagination, search, joinModelField, joinModelApiIdentifier, siblingModelOptions, inverseRelatedModelField, fieldArray, } = useHasManyThroughForm(props);
        ...
        return (React.createElement("div", { className: "flex flex-col gap-2 " },
                ...
                React.createElement("div", null,
                    React.createElement(Popover, { open: open, onOpenChange: setOpen },
                        React.createElement(PopoverTrigger, { asChild: true },
                            React.createElement("div", null,
                                React.createElement(Button, { type: "button", variant: "outline", role: "combobox", onClick: () => setOpen(!open), "aria-expanded": open, "aria-controls": listboxId, className: `w-[300px] flex flex-row items-center justify-between cursor-pointer ${open ? "bg-accent ring-ring/50 ring-3" : ""}` },
                                    React.createElement(Label, { className: "truncate grow text-left cursor-pointer" },
                                        "Add ",
                                        siblingModelName ?? "related model"),
                                    React.createElement(ChevronsUpDown, { className: "opacity-50 w-5 h-5 shrink-0" })))),
                        React.createElement(PopoverContent, { className: "w-[300px] bg-background p-0" },
            ...
            joinRecords.length > 0 ? (React.createElement("div", { className: "flex flex-col gap-3" }, joinRecords.map(([fieldKey, idx, record]) => {
                ...
                return (React.createElement("div", { className: "flex items-center w-full border rounded-md ", key: fieldKey },
                    React.createElement("div", { className: "flex flex-col w-full" },
                        React.createElement("div", { className: "flex-1 p-2" },
                            React.createElement("div", { className: "flex justify-between items-center w-full" },
                                ...
                                React.createElement("div", { className: "flex items-start self-start" },
                                    React.createElement(Button, { id: `deleteButton_${pathPrefix}.${idx}`, className: "p-0 h-4.5 w-4.5 rounded-sm shrink-0", variant: "ghost", type: "button", size: "icon", onClick: () => remove(idx) },
                                        React.createElement(X, { className: "w-4 h-4" }))))),
                        props.children && (React.createElement("div", { className: "flex-1 p-2 border-t", onClick: (e) => {
                                e.stopPropagation();
                                e.preventDefault();
                            } },
            ...
            }))) : (React.createElement("div", { className: "flex items-center justify-center border rounded-md p-2 mt-4" },
