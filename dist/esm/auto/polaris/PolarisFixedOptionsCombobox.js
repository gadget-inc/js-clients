import { Autocomplete, HorizontalStack, Icon, Tag } from "@shopify/polaris";
...
export const PolarisFixedOptionsCombobox = (props) => {
    ...
    const verticalContentMarkup = selectedOptions.length > 0 && props.allowMultiple ? (React.createElement(HorizontalStack, { gap: "2" }, selectedOptions.map((option) => {
        return (React.createElement(Tag, { key: `option-${option.value}`, onRemove: removeOption(option.value) }, option.label));
    }))) : null;
    const textField = (React.createElement(Autocomplete.TextField, { onChange: updateText, label: label, value: inputValue, prefix: allowMultiple ? undefined : React.createElement(Icon, { source: SearchMinor, color: "base" }), verticalContent: verticalContentMarkup, placeholder: "Search", autoComplete: "off" }));
    return (React.createElement(Autocomplete, { options: options, ...rest, selected: selectedValues, onSelect: updateSelection, textField: textField, allowMultiple: allowMultiple }));
};
