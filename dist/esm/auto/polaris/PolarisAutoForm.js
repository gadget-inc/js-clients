...
props) => {
    ...
    return (React.createElement(AutoFormMetadataContext.Provider, { value: { submit, metadata } },
        React.createElement(FormProvider, { ...originalFormMethods },
            React.createElement(Form, { ...rest, onSubmit: submit },
                React.createElement(FormLayout, null,
                    fields.map(([path, field]) => (React.createElement(PolarisFormInput, { key: field.apiIdentifier, path: path, field: field, control: originalFormMethods.control }))),
                    React.createElement(Button, { loading: isLoading, submit: true, onClick: async () => { submit; } }, props.submitLabel ?? "Submit"))))));
};
