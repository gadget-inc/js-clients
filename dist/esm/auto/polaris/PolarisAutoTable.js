...
export const PolarisAutoTable = (props) => {
    ...
    const [sortColumnApiIdentifier, setSortColumnApiIdentifier] = useState(props.sort ? Object.keys(props.sort)[0] : undefined);
    const [sortDirection, setSortDirection] = useState(props.sort ? Object.values(props.sort)[0] : undefined);
    ...
    return (React.createElement(BlockStack, null,
        ...
        React.createElement(IndexTable, { selectedItemsCount: selection.recordIds.length, ...disablePaginatedSelectAllButton, onSelectionChange: selection.onSelectionChange, ...polarisTableProps, bulkActions: bulkActionOptions.map((option) => ({ content: option.humanizedName, onAction: option.selectModelAction })), resourceName: resourceName, emptyState: React.createElement(EmptySearchResult, { title: `No ${resourceName.plural} yet`, description: "", withIllustration: true }), loading: fetching, hasMoreItems: page.hasNextPage, itemCount: error
