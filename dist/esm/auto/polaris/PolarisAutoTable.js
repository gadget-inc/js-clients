...
const PolarisAutoTableComponent = (props) => {
    ...
    const promotedBulkActions = useMemo(() => bulkActionOptions
    ...
    const bulkActions = useMemo(() => bulkActionOptions
        .filter((option) => !option.promoted)
        .map(bulkActionOptionMapper({
        rawRecords,
        selectedRows,
    ...
    if (!error && ((fetching && !rows) || !columns)) {
        return props.loadingSkeleton ?? React.createElement(PolarisSkeletonTable, { columns: 3 });
    }
    return (React.createElement(AutoTableContext.Provider, { value: [methods, refresh] },
        React.createElement(BlockStack, null,
            ...
            searchable && search && (React.createElement(IndexFilters, { mode: mode, setMode: setMode, appliedFilters: [], filters: [], onClearAll: () => undefined, tabs: [], canCreateNewView: false, selected: 1, loading: fetching, cancelAction: { onAction: () => search.clear() }, disabled: !!error, 
                // Search
