import { Banner, BlockStack, Box, DataTable, EmptySearchResult, Frame, IndexFilters, IndexTable, SkeletonBodyText, useSetIndexFiltersMode, } from "@shopify/polaris";
    ...
    return (React.createElement(AutoTableContext.Provider, { value: [methods, refresh] },
        React.createElement(Frame, null,
            React.createElement(BlockStack, null,
                React.createElement(PolarisAutoBulkActionModal, { model: props.model, modelActionDetails: selectedModelActionDetails, ids: selection.recordIds, selectedRows: selectedRows, clearSelection: selection.clearAll }),
                ...
                React.createElement(IndexTable, { selectedItemsCount: selection.recordIds.length, ...disablePaginatedSelectAllButton, onSelectionChange: selection.onSelectionChange, ...polarisTableProps, promotedBulkActions: promotedBulkActions.length ? promotedBulkActions : undefined, bulkActions: bulkActions.length ? bulkActions : undefined, resourceName: resourceName, emptyState: props.emptyState ?? React.createElement(EmptySearchResult, { title: `No ${resourceName.plural} yet`, description: "", withIllustration: true }), loading: fetching, hasMoreItems: page.hasNextPage, itemCount: error
                    ...
                    rows.map((row, index) => {
                        ...
                        return (React.createElement(IndexTable.Row, { key: row.id, id: row.id, position: index, onClick: onClick ? onClickCallback(row, rawRecord) : undefined, selected: selection.recordIds.includes(row.id) }, columns.map((column) => (React.createElement(IndexTable.Cell, { key: column.identifier },
                            React.createElement("div", { style: { maxWidth: "200px" } }, column.type == "CustomRenderer" ? row[column.identifier] : (React.createElement(PolarisAutoTableCellRenderer, { column: column, value: row[column.identifier] }))))))));
                    }))))));
};
