...
const PolarisAutoTableComponent = (props) => {
    ...
    const promotedBulkActions = useMemo(() => bulkActionOptions.filter((option) => option.promoted).map(bulkActionOptionMapper(selectedRows)), [bulkActionOptions, selectedRows]);
    const bulkActions = useMemo(() => bulkActionOptions.filter((option) => !option.promoted).map(bulkActionOptionMapper(selectedRows)), [bulkActionOptions, selectedRows]);
    ...
    return (React.createElement(BlockStack, null,
        ...
        searchable && (React.createElement(IndexFilters, { mode: mode, setMode: setMode, appliedFilters: [], filters: [], onClearAll: () => undefined, tabs: [], selected: 1, loading: fetching, cancelAction: { onAction: () => search.clear() }, disabled: !!error, 
            // Search
        ...
        React.createElement(IndexTable, { selectedItemsCount: selection.recordIds.length, ...disablePaginatedSelectAllButton, onSelectionChange: selection.onSelectionChange, ...polarisTableProps, promotedBulkActions: promotedBulkActions.length ? promotedBulkActions : undefined, bulkActions: bulkActions.length ? bulkActions : undefined, resourceName: resourceName, emptyState: props.emptyState ?? React.createElement(EmptySearchResult, { title: `No ${resourceName.plural} yet`, description: "", withIllustration: true }), loading: fetching, hasMoreItems: page.hasNextPage, itemCount: error
            ...
            columns &&
            rows.map((row, index) => (React.createElement(IndexTable.Row, { key: row.id, id: row.id, position: index, onClick: onClick ? onClickCallback(row) : undefined, selected: selection.recordIds.includes(row.id) }, columns.map((column) => (React.createElement(IndexTable.Cell, { key: column.field },
                React.createElement("div", { style: { maxWidth: "200px" } }, column.type == "CustomRenderer" ? row[column.field] : (React.createElement(PolarisAutoTableCellRenderer, { column: column, value: row[column.field] }))))))))))));
};
