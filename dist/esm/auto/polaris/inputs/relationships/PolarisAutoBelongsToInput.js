...
export const PolarisAutoBelongsToInput = (props) => {
    const { fieldMetadata: { path, metadata }, relatedModelOptions: { options, searchFilterOptions, pagination, search }, isLoading, errorMessage, selectedRecordId, retrievedSelectedRecordIdExists, onSelectRecord, onRemoveRecord, } = useBelongsToInputController(props);
    ...
    const selectedRecordTag = selectedRecordId ? (React.createElement(Tag, { onRemove: onRemoveRecord, key: `selectedRecordTag_${selectedRecordId}` },
        React.createElement("p", { style: { color: !retrievedSelectedRecordIdExists ? "red" : undefined }, id: `${selectedRecordId}_${selectedRecordLabel}` }, selectedRecordLabel))) : null;
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement(Combobox, { activator: React.createElement(Combobox.TextField, { prefix: React.createElement(Icon, { source: SearchIcon }), onChange: search.set, onBlur: pagination.resetPagination, value: search.value, name: path, label: metadata.name, placeholder: "Search", autoComplete: "off", verticalContent: selectedRecordTag }), onScrolledToBottom: pagination.loadNextPage, willLoadMoreOptions: pagination.hasNextPage && options.length >= optionRecordsToLoadCount },
