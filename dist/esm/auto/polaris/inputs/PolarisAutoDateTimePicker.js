...
export const PolarisAutoDateTimePicker = (props) => {
    ...
    const localTime = useMemo(() => {
        const result = value ? value : isValid(new Date(fieldProps.value)) ? new Date(fieldProps.value) : undefined;
    ...
    return (React.createElement(InlineStack, { gap: "400" },
        ...
        (props.includeTime ?? config.includeTime) && (React.createElement(PolarisAutoTimePicker, { fieldProps: fieldProps, id: props.id, localTime: localTime, onChange: onChange, hideTimePopover: props.hideTimePopover, localTz: localTz, timePickerProps: props.timePickerProps, value: value }))));
};
