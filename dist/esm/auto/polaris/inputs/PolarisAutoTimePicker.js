...
import { copyTime, getDateFromDateTimeObject, getDateTimeObjectFromDate, getTimeString, timeFormatRegex, zonedTimeToUtc, } from "../../../dateTimeUtils.js";
    ...
    return items.map((item, i) => {
        return (React.createElement("div", { key: i, style: { textAlign: "center", cursor: "default", padding: "0px 4px" } },
            React.createElement(Listbox.Option, { value: item },
                React.createElement("div", { style: { cursor: "pointer" }, onMouseEnter: () => setSelectCoord({ col: colNum, row: i }), onMouseLeave: () => setSelectCoord({ col: -1, row: -1 }) }, selectedState.padStart(2, "0") == item.padStart(2, "0") ||
                    (parseInt(selectedState, 10) == 0 && item == "12") ||
                    (selectCoord?.col == colNum && selectCoord.row == i) ? (React.createElement(Box, { padding: "100", background: "bg-inverse", borderRadius: "200", minHeight: "30px", minWidth: "30px" },
