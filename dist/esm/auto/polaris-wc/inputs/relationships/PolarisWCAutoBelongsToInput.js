 ...
 */
export const PolarisWCAutoBelongsToInput = autoInput((props) => {
    const { field, label, placeholder } = props;
    const { fieldMetadata: { metadata, path }, relatedModelOptions: { options, searchFilterOptions, pagination, search, relatedModel }, isLoading, errorMessage, selectedRecord, danglingSelectedRecordId, onSelectRecord, onRemoveRecord, } = useBelongsToInputController(props);
    ...
    const selectedRecordTag = useMemo(() => {
        const tagConfig = selectedOption && selectedOption.id
            ...
            : danglingSelectedRecordId
                ? {
                    content: React.createElement("span", { style: { color: "red" } },
                        "id: ",
                        danglingSelectedRecordId),
                    key: `selectedRecordTag_${danglingSelectedRecordId}`,
                }
                : undefined;
        return tagConfig ? (React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: "4px", marginTop: "8px" } },
            React.createElement(PolarisWCDismissibleBadge, { onDismiss: onRemoveRecord, key: tagConfig.key }, tagConfig.content))) : undefined;
    ...
    return (React.createElement(PolarisWCCombobox, { id: path, label: inputLabel, placeholder: placeholder ?? "Search", value: search.value, onChange: search.set, required: metadata.requiredArgumentForInput, onScrolledToBottom: pagination.loadNextPage, willLoadMoreOptions: pagination.hasNextPage && options.length >= optionRecordsToLoadCount, error: errorMessage, verticalContent: selectedRecordTag },
        React.createElement(PolarisWCRelatedModelOptions, { isLoading: isLoading, errorMessage: errorMessage, checkSelected: (id) => id === selectedRecord?.id, onSelect: onSelect, options: searchFilterOptions, records: relatedModel.records, canLoadMore: pagination.hasNextPage && options.length >= optionRecordsToLoadCount, onLoadMore: pagination.loadNextPage })));
});
