 ...
 */
export const PolarisWCAutoDateTimePicker = autoInput((props) => {
    ...
    const timeInputValue = localTime
        ...
        : "";
    return (React.createElement("div", { style: { display: "flex", gap: "16px", alignItems: "flex-start" } },
        React.createElement("s-date-field", { id: props.id ? `${props.id}-date` : undefined, label: typeof props.label === "string" ? props.label : String(metadata.name ?? "Date"), value: dateInputValue, required: metadata.requiredArgumentForInput, error: props.error ?? fieldState.error?.message, onChange: handleDateChange }),
        includeTime && (React.createElement("div", { style: { width: "130px" } },
            React.createElement("s-text-field", { id: props.id ? `${props.id}-time` : undefined, label: "Time", ...{ type: "time" }, value: timeInputValue, onChange: handleTimeChange })))));
});
