...
export const actionRunner = async (modelManager, operation, defaultSelection, modelApiIdentifier, modelSelectionField, isBulkAction, variables, options, namespace, hasReturnType) => {
    const plan = actionOperation(operation, defaultSelection, modelApiIdentifier, modelSelectionField, variables, options, namespace, hasReturnType);
    ...
    if (isBulkAction) {
        ...
        return !hasReturnType ? hydrateRecord(response, mutationResult[modelSelectionField]) : mutationResult.result;
    }
};
