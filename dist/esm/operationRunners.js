...
export const findOneRunner = (modelManager, operation, id, defaultSelection, modelApiIdentifier, options, throwOnEmptyData = true, namespace) => {
    ...
    return maybeLiveStream($results, (response) => {
        ...
        return hydrateRecord(response, record, modelManager);
    }, options);
};
...
const processBulkActionResponse = (defaultSelection, response, records, modelSelectionField, hasReturnType, modelManager) => {
    if (defaultSelection == null)
        return;
    if (!hasReturnType) {
        return hydrateRecordArray(response, records[modelSelectionField], modelManager);
    }
        ...
        return Object.entries(hasReturnType).flatMap(([returnTypeField, innerHasReturnType]) => {
            ...
            return results.map((result) => {
                ...
                if (!returnTypeForResult) {
                    return hydrateRecord(response, result, modelManager);
                }
                else {
...
export const processActionResponse = (defaultSelection, response, record, modelSelectionField, hasReturnType, modelManager) => {
    ...
    if (defaultSelection == null)
        return;
    if (!hasReturnType) {
        return hydrateRecord(response, record[modelSelectionField], modelManager);
    }
    else if (typeof hasReturnType == "boolean") {
...
export const backgroundActionResultRunner = async (connection, id, action, options, modelManager) => {
    ...
    switch (action.type) {
        case "action": {
            backgroundAction.result = processActionResponse(action.defaultSelection, response.data, backgroundAction.result, action.isBulk ? action.modelApiIdentifier : action.modelSelectionField, action.hasReturnType, modelManager);
            break;
        }
        case "globalAction": {
