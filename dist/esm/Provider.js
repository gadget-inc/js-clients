...
const InnerGadgetProvider = memo(({ children, gadgetAppUrl, originalQueryParams, api, type, host, isReady, isRootFrameRequest, shopifyInstallState, }) => {
    ...
    const maybeGetShopifyInstallState = useCallback(() => {
        return shopifyInstallState ?? typeof window != "undefined" ? window.gadgetConfig?.shopifyInstallState : undefined;
    }, [shopifyInstallState]);
    ...
    if (fetchOrInstallShopData) {
        ...
        const hint = maybeGetShopifyInstallState();
        if (hint) {
    ...
    useEffect(() => {
        ...
        if (hasInstallStateHint) {
            console.debug("[gadget-rsab] shopifyInstallState hint used", maybeGetShopifyInstallState());
        }
    }, [fetchingOrInstallingShop, hasInstallStateHint, maybeGetShopifyInstallState]);
...
export const Provider = ({ type, children, api, location: _location, shopifyInstallState, }) => {
    ...
    if (coalescedType == AppType.Embedded &&
