...
export const actionResultOperation = (id, action, options) => {
    let fields = {};
    switch (action.type) {
        case "action": {
            const selection = (options === null || options === void 0 ? void 0 : options.select) || action.defaultSelection;
            fields = {
                [`... on ${camelize(action.operationName)}Result`]: actionResultFieldSelection(action.modelApiIdentifier, selection, action.isBulk, action.hasReturnType),
            };
            break;
        }
        case "globalAction": {
