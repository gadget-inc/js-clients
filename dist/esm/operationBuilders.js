...
const actionResultFieldSelection = (modelSelectionField, selection, hasReturnType, depth = 0) => {
    ...
    if (hasReturnType && typeof hasReturnType != "boolean") {
        for (const [selectionField, returnTypeSelection] of Object.entries(hasReturnType)) {
            fieldSelection[selectionField] = actionResultFieldSelection(modelSelectionField, selection, returnTypeSelection.hasReturnType, depth + 1);
        }
    }
    else if (hasReturnType) {
        fieldSelection.result = true;
    }
    else if (selection) {
