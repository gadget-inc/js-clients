    ...
    function traverse(input, path = undefined, depth = 0, isInArray) {
        if (Array.isArray(input)) {
            return input.flatMap((item) => {
                ...
                console.log("where - traverse - input is an array - indexId:", indexId);
                if (!indexId) {
                    return [];
                }
            ...
            if (depth > 1) {
                ...
                console.log("where - traverse - input is an object:", isInArray, newPath, path);
                if ("id" in input) {
    ...
    function transform(input, context) {
        ...
        if (path && Array.isArray(input)) {
            ...
            if (fieldType && idPath && (fieldType.type == "HasMany" || fieldType.type == "HasManyThrough")) {
                console.log("where - transform - input is an array - gonna grab the list of ids from the updates object, based on the path:", idPath);
                edge = recordIdsAtPath[idPath]; // grab the list of ids from the updates object, based on the path
            }
            const handled = [];
            console.log("where - transform - input is an array - fieldType, edge and idPath:", fieldType, edge, idPath);
            if (edge && idPath) {
