...
export const namespacedGraphQLTypeName = (modelApiIdentifier, givenNamespaces) => {
...
export const sortTypeName = (modelApiIdentifier, namespace) => `${namespacedGraphQLTypeName(modelApiIdentifier, namespace)}Sort`;
export const filterTypeName = (modelApiIdentifier, namespace) => `${namespacedGraphQLTypeName(modelApiIdentifier, namespace)}Filter`;
...
export const hydrateRecord = (response, record, modelManager) => {
    const hydrator = getHydrator(response);
    if (hydrator) {
        record = hydrator.apply(record);
    }
    return new GadgetRecord(record, modelManager);
};
export const hydrateRecordArray = (response, records, modelManager) => {
    const hydrator = getHydrator(response);
    if (hydrator) {
        records = hydrator.apply(records);
    }
    return records === null || records === void 0 ? void 0 : records.map((record) => new GadgetRecord(record, modelManager));
};
export const hydrateConnection = (response, connection, modelManager) => {
    const nodes = connection.edges.map((edge) => edge.node);
    return hydrateRecordArray(response, nodes, modelManager);
};
...
export const namespaceDataPath = (dataPath, namespace) => {
    if (namespace) {
 ...
 **/
export const hydrationSelection = (modelApiIdentifier, namespace) => {
 ...
 **/
export function namespacify(namespace, fields) {
    if (!namespace)
        return fields;
