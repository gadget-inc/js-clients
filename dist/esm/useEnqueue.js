import { BackgroundActionHandle, disambiguateActionVariables, disambiguateBulkActionVariables, enqueueActionOperation, get, graphqlizeBackgroundOptions, namespaceDataPath, } from "@gadgetinc/api-client-core";
...
export const useEnqueue = (action, baseBackgroundOptions) => {
    ...
    const plan = useMemo(() => enqueueActionOperation(action.operationName, action.variables, action.namespace, null, action.isBulk), [action]);
    ...
    return [
        state,
        useCallback(async (input, options) => {
            const variables = action.isBulk
                ? disambiguateBulkActionVariables(action, input)
