...
function zt() {
  var e, t;
  return {
    source: Ge(Ct((i) => (e = i.next, t = i.complete, Et))),
    next(i) {
      e && e(i);
    },
    complete() {
...
`) : e, Qt = (e) => e.split(gn).map(vn).join("").trim(), Jt = /* @__PURE__ */ new Map(), Ye = /* @__PURE__ */ new Map(), Vt = (e) => {
  var t;
...
# ${i}`, t));
  ...
  var t, i;
  return typeof e == "string" ? (t = Ht(e), i = Ye.get(t) || an(e)) : (t = e.__key || Ht(e), i = Ye.get(t) || e), i.loc || Vt(i), i.__key = t, Ye.set(t, i), i;
}, dt = (e, t, i) => {
  var n = t || {}, r = Pi(e), a = et(n), o = r.__key;
  return a !== "{}" && (o = gt(a, o)), {
    key: o,
    query: r,
    variables: n,
  ...
  if (o)
    for (var u of o) {
      ...
      if (u.items)
        for (var f = +c >= 0 ? c : 0, m = 0, P = u.items.length; m < P; m++)
...
function ki(e) {
  return {
    query: e.extensions && e.extensions.persistedQuery && !e.extensions.persistedQuery.miss ? void 0 : Vt(e.query),
    operationName: Ti(e.query),
...
}, Pn = typeof TextDecoder < "u" ? new TextDecoder() : null, Tn = /boundary="?([^=";]+)"?/i, An = /data: ?([^\n]+)/, Yt = (e) => e.constructor.name === "Buffer" ? e.toString() : Pn.decode(e);
async function* Zt(e) {
  ...
  try {
    ...
    if (/multipart\/mixed/i.test(o))
      s = async function* (l, p) {
        var y = l.match(Tn), f = "--" + (y ? y[1] : "-"), m = !0, P;
...
` + f)) {
          ...
          try {
            yield P = JSON.parse(A.slice(A.indexOf(`\r
\r
`) + 4));
          } catch (E) {
            if (!P)
...
var ii = (e, t) => Ne(e.kind, e, {
...
}, Cn = ({ forward: e, dispatchDebug: t }) => (i) => {
  var n = Me((a) => {
  ...
  return Ae([n, r]);
}, Vn = (e) => ({ client: t, forward: i, dispatchDebug: n }) => e.reduceRight((r, a) => {
  var o = !1;
  return a({
    client: t,
    forward(s) {
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    m.subscribeToDebugTarget = (d) => Xe(d)(H), P = A;
  }
  var E = Vn(t.exchanges), x = Ge(E({
    client: m,
    dispatchDebug: P,
    forward: Fn({
      dispatchDebug: P
...
function ji(e) {
  ...
  switch (e.type) {
    case V.ConnectionInit:
    case V.ConnectionAck:
    case V.Ping:
    case V.Pong: {
...
function $n(e) {
  ...
  const x = E, d = (() => {
    const w = (() => {
      ...
      return {
        ...
        emit(I) {
          var C;
          "id" in I && ((C = k[I.id]) === null || C === void 0 || C.call(k, I));
        }
      };
    })(), D = {
  ...
  async function Gt() {
    clearTimeout(be);
    const [w, D] = await (h ?? (h = new Promise((C, ye) => (async () => {
      if (ct) {
        if (await l(We), !_)
      ...
      d.emit("connecting");
      const T = new x(typeof t == "function" ? await t() : t, xn);
      let ve, Ce;
      function Ke() {
        isFinite(o) && o > 0 && (clearTimeout(Ce), Ce = setTimeout(() => {
          T.readyState === x.OPEN && (T.send(Ve({ type: V.Ping })), d.emit("ping", !1, void 0));
        }, o));
      }
      b((B) => {
        h = void 0, clearTimeout(ve), clearTimeout(Ce), ye(B), Je(B) && B.code === 4499 && (T.close(4499, "Terminated"), T.onerror = null, T.onclose = null);
      }), T.onerror = (B) => d.emit("error", B), T.onclose = (B) => d.emit("closed", B), T.onopen = async () => {
        try {
          d.emit("opened", T);
          const B = typeof i == "function" ? await i() : i;
          if (T.readyState !== x.OPEN)
            return;
          T.send(Ve(B ? {
            type: V.ConnectionInit,
            payload: B
          } : {
            type: V.ConnectionInit
            // payload is completely absent if not provided
          }, A)), isFinite(u) && u > 0 && (ve = setTimeout(() => {
            T.close(J.ConnectionAcknowledgementTimeout, "Connection acknowledgement timeout");
          }, u)), Ke();
        } catch (B) {
          d.emit("error", B), T.close(J.InternalClientError, oi(B instanceof Error ? B.message : new Error(B).message, "Internal client error"));
        }
      };
      let Se = !1;
      T.onmessage = ({ data: B }) => {
        try {
          ...
          if (d.emit("message", R), R.type === "ping" || R.type === "pong") {
            d.emit(R.type, !0, R.payload), R.type === "pong" ? Ke() : s || (T.send(Ve(R.payload ? {
              type: V.Pong,
              payload: R.payload
            } : {
              type: V.Pong
          ...
          if (Se)
            return;
          if (R.type !== V.ConnectionAck)
            throw new Error(`First message cannot be of type ${R.type}`);
          clearTimeout(ve), Se = !0, d.emit("connected", T, R.payload), ct = !1, We = 0, C([
            T,
            new Promise((Oa, Mi) => b(Mi))
          ]);
        } catch (R) {
          T.onmessage = null, d.emit("error", R), T.close(J.BadResponse, oi(R instanceof Error ? R.message : new Error(R).message, "Bad response"));
        }
      };
    })())));
    ...
    return [
      ...
      Promise.race([
        ...
        I.then(() => {
          if (!_) {
            const C = () => w.close(1e3, "Normal Closure");
            isFinite(a) && a > 0 ? be = setTimeout(() => {
              w.readyState === x.OPEN && C();
            }, a) : C();
          }
        }),
        // or
        ...
        for (_++; ; )
          try {
            ...
            T.send(Ve({
              ...
              !I && T.readyState === x.OPEN && T.send(Ve({
                id: k,
                type: V.Complete
              }, A)), _--, I = !0, ve();
            }, await Ce.finally(Ke);
...
`)), e.error;
  ...
  if (!i.success) {
    ...
    throw n ? Ci(n) : new st("Gadget API operation not successful.", "GGT_UNKNOWN");
  }
  return i;
}, Vi = (e) => {
  var t, i, n, r;
...
class sr {
        ...
        try {
          p = await this.waitForOpenedConnection({
            isFatalConnectionProblem(P) {
              return console.warn("Transport error encountered during transaction processing", P), !0;
            },
          ...
          const f = new ai({
            ...
            exchanges: [
              ...
              ri({
                forwardSubscription(P) {
                  const A = { ...P, query: P.query || "" };
                  return {
                    subscribe: (H) => ({
  ...
  newBaseClient() {
    ...
    typeof window < "u" && t.push(_n), t.push(...this.exchanges.beforeAsync, Cn, ri({
...
class Be extends Array {
  ...
  async nextPage() {
    var t;
    if (!this.hasNextPage)
      throw new Tt("Cannot request next page because there isn't one, should check 'hasNextPage' to see if it exists");
  ...
  async previousPage() {
    ...
    if (!this.hasPreviousPage)
      throw new Tt("Cannot request previous page because there isn't one, should check 'hasPreviousPage' to see if it exists");
...
class le {
  ...
  async bulkCreate(t) {
    ...
    if (!(!((i = this.options) === null || i === void 0) && i.pluralApiIdentifier))
      throw new Tt("Cannot perform bulkCreate without a pluralApiIdentifier");
