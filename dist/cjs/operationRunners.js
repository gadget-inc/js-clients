...
const actionResultRunner = (connection, id, action, options) => {
    ...
    return new Promise((resolve) => {
        ...
        subscription.subscribe((response) => {
            ...
            if (!response.data)
                return;
            const backgroundAction = (0, support_js_1.assertOperationSuccess)(response, ["data", "backgroundAction"]);
            console.log("backgroundAction", backgroundAction);
            if (backgroundAction.outcome) {
                (0, support_js_1.assertResponseSuccess)(backgroundAction.result);
                console.log("result", backgroundAction.result);
                switch (action.type) {
                    case "action": {
                        backgroundAction.result = processActionResponse(action.defaultSelection, response, (0, support_js_1.get)(backgroundAction.result, [action.modelSelectionField]), action.hasReturnType);
                        break;
                    }
                    case "globalAction": {
