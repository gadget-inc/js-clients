...
const findOneByFieldRunner = async (modelManager, operation, fieldName, fieldValue, defaultSelection, modelApiIdentifier, options) => {
    ...
    const records = (0, support_js_1.hydrateConnection)(response, connectionObject, modelManager);
    if (records.length > 1) {
...
const actionRunner = async (modelManager, operation, defaultSelection, modelApiIdentifier, modelSelectionField, isBulkAction, variables, options, namespace) => {
    const plan = (0, operationBuilders_js_1.actionOperation)(operation, defaultSelection, modelApiIdentifier, modelSelectionField, variables, options, namespace);
    ...
    if (isBulkAction) {
        return (0, support_js_1.hydrateRecordArray)(response, mutationResult[modelSelectionField], modelManager);
    }
    else {
        return (0, support_js_1.hydrateRecord)(response, mutationResult[modelSelectionField], modelManager);
    }
};
