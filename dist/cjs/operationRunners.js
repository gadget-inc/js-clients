...
async function enqueueActionRunner(connection, action, variables, options = {}) {
    ...
    try {
        ...
        if ("isBulk" in action && action.isBulk) {
...
const actionResultRunner = async (connection, id, action, options) => {
    ...
    switch (action.type) {
        case "action": {
            backgroundAction.result = processActionResponse(action.defaultSelection, response.data, backgroundAction.result, action.modelSelectionField, action.hasReturnType);
            break;
        }
        case "globalAction": {
