...
const actionResultRunner = async (connection, id, action, options) => {
    ...
    if (backgroundAction.outcome) {
        switch (action.type) {
            case "action": {
                backgroundAction.result = processActionResponse(action.defaultSelection, response, (0, support_js_1.get)(response, [action.modelSelectionField]), action.hasReturnType);
            }
        }
    }
