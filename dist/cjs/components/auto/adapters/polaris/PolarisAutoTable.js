...
const PolarisAutoTable = (props) => {
    const [{ rows, columns, metadata, fetching, page, error }, refresh] = (0, useTable_1.useTable)(props.manager, {
        select: props.select,
    });
    const polarisTableProps = (0, react_1.useMemo)(() => {
        ...
        if (columns) {
            for (const column of columns) {
                columnContentTypes.push(column.fieldType == metadata_1.FieldType.Number ? "numeric" : "text");
    ...
    if (error) {
        return react_1.default.createElement(PolarisErrorDisplay_1.PolarisErrorDisplay, { error: error, retry: refresh });
    }
