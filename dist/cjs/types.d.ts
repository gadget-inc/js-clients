...
import type { ActionFunction, ActionFunctionMetadata, AnyActionFunction, BulkActionFunction, GlobalActionFunction, ViewFunction, ViewFunctionWithoutVariables, ViewFunctionWithVariables } from "./GadgetFunctions.js";
 ...
 */
export type DefaultSelection<Available extends FieldSelection, Options extends {
    select?: Available | null;
}, Defaults extends SomeFieldsSelected<Available>> = IfAny<Options, Defaults, IfAny<Options["select"], Defaults, Options["select"] extends Available ? Options["select"] : Defaults>>;
/**
 ...
 * ```
 */
type InnerSelect<Schema, Selection extends FieldSelection | FieldSelectionWithArgs | null | undefined> = IfAny<Selection, never, Selection extends null | undefined ? never : Schema extends (infer T)[] ? InnerSelect<T, Selection>[] : Schema extends null ? InnerSelect<Exclude<Schema, null>, Selection> | null : {
    [Key in Exclude<keyof Selection, typeof $args> & keyof Schema]: Selection[Key] extends true ? Schema[Key] : Selection[Key] extends FieldSelection | FieldSelectionWithArgs ? Exclude<keyof Selection[Key], typeof $args> extends never ? Schema[Key] : InnerSelect<Schema[Key], Selection[Key]> : never;
}>;
/**
 ...
 * ```
 */
export type Select<Schema, Selection extends FieldSelection | FieldSelectionWithArgs | null | undefined> = DeepFilterNever<InnerSelect<Schema, Selection>>;
 ...
 * }
 */
export type AvailableSelection<Schema> = Schema extends Array<infer T> ? AvailableSelection<T> : Schema extends object ? {
...
export interface AnyBackgroundActionHandle<SchemaT, Action extends ActionFunctionMetadata<any, any, any, SchemaT, any, any> | GlobalActionFunction<any>> {
    readonly id: string;
...
export type ViewResult<F extends ViewFunction<any, any>> = Awaited<F extends ViewFunctionWithVariables<any, infer Result> ? Result : F extends ViewFunctionWithoutVariables<infer Result> ? Result : never>;
