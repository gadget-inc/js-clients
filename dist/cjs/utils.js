...
exports.transformData = exports.hasNested = exports.set = exports.get = exports.disambiguateActionVariables = exports.useQueryArgs = exports.useMemoizedQueryOptions = exports.ErrorWrapper = exports.noProviderErrorMessage = void 0;
...
function transformData(defaultValues, data) {
    const updates = getUpdates(defaultValues);
    function transform(input, updates, depth = 0, path = undefined, isInArray = false, isParentAnObject = false) {
        if (Array.isArray(input)) {
            ...
            results.push(input.filter((_item, index) => !handled.includes(index)).map((item, index) => {
                const currentPath = path ? `${path}.${index}` : index.toString();
                return transform(item, updates, depth + 1, currentPath, true, false);
            }));
