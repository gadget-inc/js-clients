...
exports.createUseFindBy = void 0;
const operationBuilders_js_1 = require("../operationBuilders.js");
const support_js_1 = require("../support.js");
const utils_js_1 = require("./utils.js");
const createUseFindBy = (adapter, coreHooks) => {
    const useFindBy = (finder, value, options) => {
        const memoizedOptions = coreHooks.useStructuralMemo(options);
        const plan = adapter.framework.useMemo(() => {
            return (0, operationBuilders_js_1.findOneByFieldOperation)(finder.operationName, finder.findByVariableName, value, finder.defaultSelection, finder.modelApiIdentifier, memoizedOptions, finder.namespace);
        ...
        const result = adapter.framework.useMemo(() => {
            const dataPath = (0, support_js_1.namespaceDataPath)([finder.operationName], finder.namespace);
            let data = rawResult.data;
            let records = [];
            if (data) {
                const connection = (0, support_js_1.get)(rawResult.data, dataPath);
                if (connection) {
                    records = (0, support_js_1.hydrateConnection)(rawResult, connection);
                    data = records[0];
                }
            }
            ...
            if (!error) {
                if (records.length > 1) {
                    error = utils_js_1.ErrorWrapper.forClientSideError((0, support_js_1.getNonUniqueDataError)(finder.modelApiIdentifier, finder.findByVariableName, value));
                }
                else if (rawResult.data && !records[0]) {
                    error = utils_js_1.ErrorWrapper.forClientSideError(new support_js_1.GadgetNotFoundError(`${finder.modelApiIdentifier} record with ${finder.findByVariableName}=${value} not found`));
                }
            }
...
exports.createUseFindBy = createUseFindBy;
