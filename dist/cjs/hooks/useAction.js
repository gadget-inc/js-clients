...
exports.createUseAction = void 0;
...
const createUseAction = (adapter, coreHooks) => {
    const useAction = (action, options) => {
        ...
        return [
            ...
            adapter.framework.useCallback(async (input, context) => {
                var _a;
                const variables = (0, support_js_1.disambiguateActionVariables)(action, input);
    ...
    if (result.data) {
        ...
        if (mutationData) {
            ...
            if (errors && errors[0]) {
                ...
                data = (0, operationRunners_js_1.processActionResponse)(action.defaultSelection, result, mutationData, action.modelSelectionField, action.hasReturnType);
            }
        }
    }
