...
const useEnqueue = (action, baseBackgroundOptions) => {
    ...
    const plan = (0, react_1.useMemo)(() => (0, api_client_core_1.enqueueActionOperation)(action.operationName, action.variables, action.namespace), [action]);
    ...
    return [
        state,
        (0, react_1.useCallback)(async (input, backgroundOptions, context) => {
            const variables = "isBulk" in action && action.isBulk ? (0, api_client_core_1.disambiguateBulkActionVariables)(action, input) : (0, api_client_core_1.disambiguateActionVariables)(action, input);
            variables.backgroundOptions = (0, api_client_core_1.graphqlizeBackgroundOptions)({ ...baseBackgroundOptions, ...backgroundOptions });
    ...
    if (data) {
        ...
        if (mutationData) {
            ...
            if (errors && errors[0]) {
                ...
                handle = new api_client_core_1.BackgroundActionHandle(connection, mutationData.backgroundAction.id, {});
            }
        }
    }
