...
class GadgetRecordList extends Array {
    ...
    firstOrThrow() {
        if (!this[0]) {
            throw new support_js_1.GadgetOperationError("No records found.", "GGT_RECORD_NOT_FOUND");
        }
        return this[0];
    }
    toJSON() {
    ...
    async nextPage() {
        var _a;
        if (!this.hasNextPage)
            throw new support_js_1.GadgetClientError("Cannot request next page because there isn't one, should check 'hasNextPage' to see if it exists");
    ...
    async previousPage() {
        ...
        if (!this.hasPreviousPage)
            throw new support_js_1.GadgetClientError("Cannot request previous page because there isn't one, should check 'hasPreviousPage' to see if it exists");
        // Our current implementation of paging determines paging direction based on if "first" is defined. We can pass both "before" and "after" as options but only after is respected if first is sent. One of "before" or "after" is ignored depending on whether "first" is defined.
