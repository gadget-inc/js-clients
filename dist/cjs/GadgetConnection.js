...
const liveQueryExchange = ({ forward }) => {
    ...
    return operations$ => {
        const notLive = (0, wonka_1.pipe)(operations$, (0, wonka_1.filter)(op => !op.query.definitions.some(isLiveQueryOperationDefinitionNode)));
        const live = (0, wonka_1.pipe)(operations$, (0, wonka_1.filter)(op => op.query.definitions.some(isLiveQueryOperationDefinitionNode)), (0, wonka_1.filter)(op => !executed.has(op.key) || op.kind !== "query"), // We will forward on everything that hasn't been executed and is not a query
        (0, wonka_1.tap)((op) => {
            if (op.kind == "query") {
                executed.add(op.key);
            }
