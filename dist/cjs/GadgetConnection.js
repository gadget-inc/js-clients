...
})(GadgetGraphQLCloseCode = exports.GadgetGraphQLCloseCode || (exports.GadgetGraphQLCloseCode = {}));
...
class GadgetConnection {
    constructor(options) {
        ...
        if (options.fetchImplementation) {
            ...
            this._fetchImplementation = async (...args) => {
                const { fetch } = await Promise.resolve().then(() => __importStar(require("cross-fetch")));
    ...
    async getCurrentModels() {
        ...
        try {
            const result = await this.currentClient.query((0, core_1.gql) `{
        gadgetMeta {
          allHydrations
        }
      }`, {});
            this.allModels = (0, support_js_1.hydrateAllModels)(result, this.allModels);
            return this.allModels;
        }
        catch (err) {
            console.error(err);
            ...
            forwardSubscription: (request) => {
                return {
                    subscribe: (sink) => {
                        const input = { ...request, query: request.query || "" };
                        return {
                            unsubscribe: (0, push_pull_async_iterable_iterator_1.applyAsyncIterableIteratorToSink)((0, graphql_live_query_patch_jsondiffpatch_1.applyLiveQueryJSONDiffPatch)((0, push_pull_async_iterable_iterator_1.makeAsyncIterableIteratorFromSink)((sink) => this.getBaseSubscriptionClient().subscribe(input, sink))), sink),
                        };
                    },
                };
            },
