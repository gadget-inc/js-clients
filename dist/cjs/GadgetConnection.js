    ...
    newBaseClient() {
        ...
        (0, core_1.subscriptionExchange)({
            isSubscriptionOperation: (request) => {
                return request.query.definitions.some((definition) => (0, index_js_1.isLiveQueryOperationDefinitionNode)(definition));
            },
            forwardSubscription: (request) => {
                return {
                    subscribe: (sink) => {
        ...
        client.reexecuteOperation = (operation) => {
            if (operation.query.definitions.some(index_js_1.isLiveQueryOperationDefinitionNode)) {
                // live queries don't properly clean up after themselves when
                // reexecuted, plus they should never need to be reexecuted, so
                // we noop
                return;
            }
