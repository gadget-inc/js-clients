...
const InnerGadgetProvider = (0, react_2.memo)(({ children, gadgetAppUrl, originalQueryParams, api, type, host, isReady, isRootFrameRequest }) => {
    ...
    const apiKey = appBridge?.config.apiKey;
    (0, react_2.useEffect)(() => {
        if (!appBridge) {
    ...
    let isShopifyManagedInstallation = false;
    ...
    const [{ data: attemptedInstallResult, fetching: attemptingInstallByTokenExchange }, attemptInstallByTokenExchange] = (0, react_1.useMutation)(InstallByTokenExchangeMutation);
    const tokenExchangeAttempted = !!attemptedInstallResult;
    if (currentSessionData) {
        if (currentSessionData.shopifyConnection) {
            requiresReauthentication = currentSessionData.shopifyConnection.requiresReauthentication;
            isShopifyManagedInstallation = (currentSessionData.shopifyConnection.connectedApps ?? []).some((app) => app.apiKey === apiKey && app.shopifyManagedInstallation);
        }
    ...
    (0, react_2.useEffect)(() => {
        if (!requiresReauthentication || !tokenExchangeAttempted || isRootFrameRequest || isShopifyManagedInstallation)
            return;
