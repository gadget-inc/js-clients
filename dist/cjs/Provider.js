...
const FetchOrInstallShopMutation = `
  mutation ShopifyFetchOrInstallShop($shopifySessionToken: String!) {
    shopifyFetchOrInstallShop(shopifySessionToken: $shopifySessionToken) {
      isAuthenticated
      redirectToOauth
    }
  }
...
const InnerGadgetProvider = (0, react_2.memo)(({ children, gadgetAppUrl, originalQueryParams, api, type, host, isReady, isRootFrameRequest }) => {
    ...
    const [{ data: fetchOrInstallShopData, fetching: fetchingOrInstallingShop, error }, fetchOrInstallShop] = (0, react_1.useMutation)(FetchOrInstallShopMutation);
    if (fetchOrInstallShopData) {
        console.debug({ fetchOrInstallShop }, "[gadget-rsab] fetched or installed shop data");
        redirectToOauth = fetchOrInstallShopData.fetchOrInstallShop.redirectToOauth;
        isAuthenticated = !fetchOrInstallShopData.fetchOrInstallShop.isAuthenticated;
    }
    ...
    (0, react_2.useEffect)(() => {
        ...
        appBridge
            ...
            fetchOrInstallShop({ shopifySessionToken }).catch((err) => {
                console.error({ err }, "[gadget-rsab] error fetching or installing shop");
            });
        })
            .catch(() => {
            console.error("[gadget-rsab] failed to get id token from app bridge");
        });
