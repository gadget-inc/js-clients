...
const recordToRow = (spec, record, index) => {
    const row = {
        id: record.id,
    };
    for (const targetColumn of spec.targetColumns) {
        if (isCustomCellColumn(targetColumn)) {
            ...
            row[targetColumn.header] = react_1.default.createElement(CellComponent, { record: record, index: index });
            continue;
        }
        const { field: columnPath } = getCellDetailColumnByColumnValue(targetColumn);
...
const fieldMetadataArrayToFieldMetadataTree = (fieldMetadataArray) => {
    ...
    for (const field of fieldMetadataArray) {
        ...
        if (isHasOneOrBelongsToField(field)) {
            map[field.apiIdentifier] = {
                $field: field,
                ...(0, exports.fieldMetadataArrayToFieldMetadataTree)(getRelatedModelFields(field)),
            };
        }
            ...
            map[field.apiIdentifier] = {
                $field: field,
                edges: {
                    node: (0, exports.fieldMetadataArrayToFieldMetadataTree)(getRelatedModelFields(field)),
                },
            };
        }
        else {
