...
const useMaybeFindFirst = (manager, options) => {
    ...
    const memoizedOptions = (0, useStructuralMemo_js_1.useStructuralMemo)(firstOptions);
    const plan = (0, react_1.useMemo)(() => {
        return (0, api_client_core_1.findManyOperation)(manager.findFirst.operationName, manager.findFirst.defaultSelection, manager.findFirst.modelApiIdentifier, memoizedOptions, manager.findFirst.namespace);
    }, [manager, memoizedOptions]);
    const [rawResult, refresh] = (0, useGadgetQuery_js_1.useGadgetQuery)((0, utils_js_1.useQueryArgs)(plan, firstOptions));
    const result = (0, react_1.useMemo)(() => {
        const dataPath = (0, api_client_core_1.namespaceDataPath)([manager.findFirst.operationName], manager.findFirst.namespace);
        let data = rawResult.data ?? null;
        if (data) {
            const connection = (0, api_client_core_1.get)(rawResult.data, dataPath);
            if (connection) {
                data = (0, api_client_core_1.hydrateConnection)(rawResult, connection, manager)[0] ?? null;
            }
            else {
                data = data[0] ?? null;
            }
