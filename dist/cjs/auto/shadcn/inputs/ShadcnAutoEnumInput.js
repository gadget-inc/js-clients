...
const makeShadcnAutoEnumInput = ({ Badge, Button, Command, CommandItem, CommandInput, Label, CommandList, CommandEmpty, CommandGroup, Checkbox, }) => {
    ...
    function ShadcnAutoEnumInput(props) {
        ...
        const { allowMultiple, allowOther, onSelectionChange, selectedOptions, allOptions, filteredOptions, searchQuery, label, path, metadata, isError, errorMessage, } = (0, useEnumInputController_js_1.useEnumInputController)({ field: fieldApiIdentifier, control });
        ...
        return (react_1.default.createElement(ShadcnComboInput, { ...props, options: filteredOptions.map((option) => ({ id: option, primary: option })), path: path, metadata: metadata, label: labelProp ?? label, onChange: debouncedSearch, selectedRecordTag: selectedTagsElement, onSelect: (option) => {
