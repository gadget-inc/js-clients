...
const PolarisAutoInput = (props) => {
    ...
    switch (config.fieldType) {
        ...
        case metadata_js_1.FieldType.Url: {
            return (react_1.default.createElement(polaris_1.TextField, { label: fieldMetadata.name, type: FieldTypeToInputType[fieldMetadata.fieldType], autoComplete: "off", ...field, error: error?.message }));
        }
        case metadata_js_1.FieldType.Boolean: {
            return react_1.default.createElement(polaris_1.Checkbox, { label: fieldMetadata.name, ...field, error: error?.message });
        }
        case metadata_js_1.FieldType.DateTime: {
            return (react_1.default.createElement(PolarisDateTimePicker_js_1.PolarisDateTimePicker, { dateLabel: fieldMetadata.name, includeTime: config.includeTime, ...field, error: error?.message }));
        }
        case metadata_js_1.FieldType.Json: {
            return react_1.default.createElement(PolarisJSONInput_js_1.PolarisJSONInput, { label: fieldMetadata.name, multiline: 4, monospaced: true, autoComplete: "off", ...field });
        }
        case metadata_js_1.FieldType.Enum: {
            const config = fieldMetadata.configuration;
            return (react_1.default.createElement(PolarisFixedOptionsCombobox_js_1.PolarisFixedOptionsCombobox, { label: fieldMetadata.name, options: config.options.map((option) => ({ value: option.name, label: option.name })), allowMultiple: config.allowMultiple, ...field }));
        }
        case metadata_js_1.FieldType.File: {
            return react_1.default.createElement(PolarisFileInput_js_1.PolarisFileInput, { label: fieldMetadata.name, ...field });
        }
        case metadata_js_1.FieldType.RoleAssignments: {
            return react_1.default.createElement(PolarisRolesCombobox_js_1.PolarisRolesCombobox, { label: fieldMetadata.name, ...field });
        }
        ...
        case metadata_js_1.FieldType.BelongsTo: {
            return react_1.default.createElement(PolarisBelongsToInput_js_1.PolarisBelongsToInput, { field: fieldMetadata.apiIdentifier });
        }
