...
const maybeGetColumnIndex = (columns, apiIdentifier) => {
...
const PolarisAutoTableComponent = (props) => {
    ...
    const polarisTableProps = (0, react_1.useMemo)(() => {
        ...
        if (columns) {
            for (const column of columns) {
                headings.push({
                    title: column.header, // Polaris type says that this must be a string, but this works with a ReactNode
                });
    ...
    const promotedBulkActions = (0, react_1.useMemo)(() => bulkActionOptions
        .filter((option) => option.promoted)
        .map(bulkActionOptionMapper({
        rawRecords,
        selectedRows,
        clearSelection: selection.clearAll,
    })), [bulkActionOptions, selectedRows, rawRecords, selection.clearAll]);
    const bulkActions = (0, react_1.useMemo)(() => bulkActionOptions
        .filter((option) => !option.promoted)
        .map(bulkActionOptionMapper({
        rawRecords,
        selectedRows,
    ...
    if (!error && ((fetching && !rows) || !columns)) {
        return props.loadingSkeleton ?? react_1.default.createElement(PolarisSkeletonTable, { columns: 3 });
    }
    return (react_1.default.createElement(AutoTableContext_js_1.AutoTableContext.Provider, { value: [methods, refresh] },
        react_1.default.createElement(polaris_1.BlockStack, null,
            ...
            searchable && search && (react_1.default.createElement(polaris_1.IndexFilters, { mode: mode, setMode: setMode, appliedFilters: [], filters: [], onClearAll: () => undefined, tabs: [], canCreateNewView: false, selected: 1, loading: fetching, cancelAction: { onAction: () => search.clear() }, disabled: !!error, 
                // Search
            ...
            react_1.default.createElement(polaris_1.IndexTable, { selectedItemsCount: selection.recordIds.length, ...disablePaginatedSelectAllButton, onSelectionChange: selection.onSelectionChange, ...polarisTableProps, promotedBulkActions: promotedBulkActions.length ? promotedBulkActions : undefined, bulkActions: bulkActions.length ? bulkActions : undefined, resourceName: resourceName, emptyState: props.emptyState ?? react_1.default.createElement(polaris_1.EmptySearchResult, { title: `No results`, description: "", withIllustration: true }), loading: fetching, hasMoreItems: page.hasNextPage, itemCount: error
                    ...
                    : undefined, sortDirection: gadgetToPolarisDirection(sort.direction), sortColumnIndex: columns ? maybeGetColumnIndex(columns, sort.column) : undefined, onSort: (headingIndex) => handleColumnSort(headingIndex), selectable: props.selectable === undefined ? bulkActionOptions.length !== 0 : props.selectable, lastColumnSticky: props.lastColumnSticky, hasZebraStriping: props.hasZebraStriping, condensed: props.condensed },
...
const bulkActionOptionMapper = (props) => {
    return (option) => ({
        ...
        onAction: (0, useTableBulkActions_js_1.getBulkActionOptionCallback)({ option, ...props }),
    });
};
