...
const PolarisAutoTableComponent = (props) => {
    ...
    const promotedBulkActions = (0, react_1.useMemo)(() => bulkActionOptions.filter((option) => option.promoted).map(bulkActionOptionMapper(selectedRows)), [bulkActionOptions, selectedRows]);
    const bulkActions = (0, react_1.useMemo)(() => bulkActionOptions.filter((option) => !option.promoted).map(bulkActionOptionMapper(selectedRows)), [bulkActionOptions, selectedRows]);
...
const bulkActionOptionMapper = (selectedRows) => {
    return (option) => ({
