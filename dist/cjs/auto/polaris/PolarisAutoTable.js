...
const PolarisAutoTableComponent = (props) => {
    ...
    return (react_1.default.createElement(polaris_1.BlockStack, null,
        ...
        react_1.default.createElement(polaris_1.IndexTable, { selectedItemsCount: selection.recordIds.length, ...disablePaginatedSelectAllButton, onSelectionChange: selection.onSelectionChange, ...polarisTableProps, bulkActions: bulkActionOptions.map((option) => ({
            ...
            })), resourceName: resourceName, emptyState: react_1.default.createElement(polaris_1.EmptySearchResult, { title: `No ${resourceName.plural} yet`, description: "", withIllustration: true }), loading: fetching, hasMoreItems: page.hasNextPage, itemCount: error
                ...
                : undefined, sortDirection: gadgetToPolarisDirection(sort.direction), sortColumnIndex: columns ? getColumnIndex(columns, sort.column) : undefined, onSort: (headingIndex) => handleColumnSort(headingIndex) }, rows &&
            columns &&
            rows.map((row, index) => (react_1.default.createElement(polaris_1.IndexTable.Row, { key: row.id, id: row.id, position: index, onClick: onClick ? onClickCallback(row) : undefined, selected: selection.recordIds.includes(row.id) }, columns.map((column) => (react_1.default.createElement(polaris_1.IndexTable.Cell, { key: column.apiIdentifier },
                react_1.default.createElement("div", { style: { maxWidth: "200px" } }, column.isCustomCell ? row[column.apiIdentifier] : (react_1.default.createElement(PolarisAutoTableCellRenderer_js_1.PolarisAutoTableCellRenderer, { column: column, value: row[column.apiIdentifier] }))))))))))));
};
