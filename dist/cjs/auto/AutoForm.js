...
const useAutoForm = (props) => {
    ...
    if (!fetchingMetadata) {
        ...
        const requiredFields = fields.filter(({ metadata }) => metadata.requiredArgumentForInput);
        if (props.exclude) {
            props.exclude.forEach((excludedField) => {
                if (requiredFields.some((field) => field.metadata.apiIdentifier === excludedField)) {
