...
const useFormFields = (metadata, options) => {
    return (0, react_1.useMemo)(() => {
        if (!metadata)
            return [];
        const action = (0, metadata_js_1.isActionMetadata)(metadata) ? metadata.action : metadata;
        const objectFields = metadata.__typename === "GadgetGlobalAction"
            ? []
            : action.inputFields.filter((field) => field.configuration.__typename === "GadgetObjectFieldConfig" &&
                (metadata.__typename === "GadgetModel" ? field.apiIdentifier === metadata.apiIdentifier : true));
