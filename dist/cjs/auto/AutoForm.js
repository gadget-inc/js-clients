...
const useAutoForm = (props) => {
    ...
    const defaultValues = (0, react_1.useMemo)(() => {
        let mergedDefaultValues = props.defaultValues;
        if (action.type === "action" && record) {
            const defaultValuesForModel = props.defaultValues?.[modelApiIdentifier] ?? {};
            const defaultValuesForRecord = (0, utils_js_1.toDefaultValues)(undefined, record);
            ...
            mergedDefaultValues = {
                ...
                [modelApiIdentifier]: {
                    ...
                    ...Object.fromEntries(Object.keys(defaultValuesForRecord).flatMap((key) => props.defaultValues && key in props.defaultValues ? [[key, props.defaultValues[key]]] : [])),
    ...
    const pauseExistingRecordLookup = record || !("findBy" in props)
    ...
    return {
        ...
        fetchingMetadata,
        metadataError,
        fields: fields.filter((field) => falseForIdFieldsUnlessUpsertWithoutFindBy({ isUpsertWithFindBy, metadata: field.metadata }) &&
        ...
        if (react_1.default.isValidElement(child)) {
            ...
            if ((0, AutoInput_js_1.isAutoInput)(child)) {
                ...
                if (props.recordLabel) {
                    aggregatePathsFromRecordLabel(props.recordLabel, () => newCurrentPath ? (getFieldsToSelectOnRecordLabelCallback?.(newCurrentPath) ?? []) : []).forEach((path) => paths.add(`${field}.${path}`));
                }
            }
...
const falseForIdFieldsUnlessUpsertWithoutFindBy = (props) => {
    const { isUpsertWithFindBy, metadata } = props;
    return metadata.fieldType === metadata_js_1.FieldType.Id ? !isUpsertWithFindBy : true;
};
/**
 * Removes `hasMany` fields that emerge from `hasManyThrough` fields that are not actually model fields
 */
const isJoinModelHasManyField = (field) => field.fieldType === metadata_js_1.FieldType.HasMany &&
