...
const useAutoForm = (props) => {
    ...
    console.log("isUpsertMetaAction :", isUpsertMetaAction, isUpsertMetaAction && !!findBy);
    const defaultValues = (0, react_1.useMemo)(() => props.defaultValues ??
        (action.type === "globalAction"
            ? {}
            : {
                [modelApiIdentifier]: {
                    ...(record ?? (!operatesWithRecordId && metadata && (0, metadata_js_1.isActionMetadata)(metadata) && metadata?.defaultRecord)),
                    ...(isUpsertMetaAction && !!findBy && { id: findBy }),
                },
    ...
    const { submit, error: formError, reset, getValues, formState: { isSubmitSuccessful, isLoading, isReady, isSubmitting, touchedFields, errors }, originalFormMethods, } = (0, useActionForm_js_1.useActionForm)(action, {
