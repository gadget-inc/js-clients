...
const useAutoForm = (props) => {
    ...
    const { submit, error: formError, formState: { isSubmitSuccessful, isLoading, dirtyFields, errors }, originalFormMethods, } = (0, useActionForm_js_1.useActionForm)(action, {
        defaultValues: {
        ...
        send: () => {
            const fieldsToSend = fields.map(({ path }) => path);
    ...
    console.log("where - form errors:", errors);
    return {
        metadata,
        fetchingMetadata,
        metadataError,
