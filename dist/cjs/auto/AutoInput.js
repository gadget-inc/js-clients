...
function autoRelationshipForm(Component) {
    const WrappedComponent = (props) => {
        const { hasCustomFormChildren, registerFields, fieldSet } = (0, AutoFormContext_js_1.useFieldsFromChildComponents)();
        const displayedRecordPaths = (0, useSelectedPathsFromRecordLabel_js_1.useSelectedPathsFromRecordLabel)(props);
        const relationshipTransformedPaths = (0, useFieldMetadata_js_1.useRelationshipTransformedMetaDataPaths)(props.field);
        const displayedRecordPathsToRegister = (0, react_1.useMemo)(() => displayedRecordPaths.map((path) => `${relationshipTransformedPaths?.metaDataPath ?? props.field}.${path}`), [displayedRecordPaths, props.field, relationshipTransformedPaths?.metaDataPath]);
        (0, react_1.useEffect)(() => {
