...
const useOptionLabelForField = (field, optionLabel) => {
    const { metadata } = (0, useFieldMetadata_js_1.useFieldMetadata)(field);
    const relationshipFieldConfig = metadata.configuration;
    return (0, client_hooks_1.assert)(optionLabel ?? relationshipFieldConfig.relatedModel?.defaultDisplayField.apiIdentifier, "Option label is required for relationships");
};
    ...
    return {
        ...
        searchFilterOptions: options.filter((option) => {
            const optionAsString = typeof option.primary === "string"
                ...
                : react_1.default.isValidElement(option.primary)
...
const useAllRelatedModelRecords = (props) => {
    ...
    const [{ data: newlyFetchedRecords, fetching, error }, _refetch] = (0, hooks_js_1.useFindMany)(relatedModelManager, {
