...
const useFindBy = (finder, value, options) => {
    ...
    const plan = (0, react_1.useMemo)(() => {
        return (0, api_client_core_1.findOneByFieldOperation)(finder.operationName, finder.findByVariableName, value, finder.defaultSelection, finder.modelApiIdentifier, memoizedOptions, finder.namespace);
    }, [finder, value, memoizedOptions]);
    const [rawResult, refresh] = (0, useGadgetQuery_js_1.useGadgetQuery)((0, utils_js_1.useQueryArgs)(plan, options));
    const result = (0, react_1.useMemo)(() => {
        const dataPath = (0, api_client_core_1.namespaceDataPath)([finder.operationName], finder.namespace);
        let data = rawResult.data;
        let records = [];
        if (data) {
            const connection = (0, api_client_core_1.get)(rawResult.data, dataPath);
            if (connection) {
                records = (0, api_client_core_1.hydrateConnection)(rawResult, connection, finder.modelManager);
                data = records[0];
            }
        }
        let error = utils_js_1.ErrorWrapper.forMaybeCombinedError(rawResult.error);
        if (!error) {
            if (records.length > 1) {
                error = utils_js_1.ErrorWrapper.forClientSideError((0, api_client_core_1.getNonUniqueDataError)(finder.modelApiIdentifier, finder.findByVariableName, value));
            }
            else if (rawResult.data && !records[0]) {
                error = utils_js_1.ErrorWrapper.forClientSideError(new api_client_core_1.GadgetNotFoundError(`${finder.modelApiIdentifier} record with ${finder.findByVariableName}=${value} not found`));
            }
        }
