...
const useHasManyThroughForm = (props) => {
    ...
    return {
        ...
        metadata,
        joinModelField,
        joinModelApiIdentifier: (0, client_hooks_1.assert)(joinModelApiIdentifier, `The join model in the "${field}" hasManyThrough field is required`),
        inverseRelatedModelField: (0, client_hooks_1.assert)(inverseRelatedModelField, `The belongsTo field between the join model and sibling model in the "${field}" hasManyThrough field is required`),
        fieldArrayPath,
        fieldArray,
        field,
        records,
