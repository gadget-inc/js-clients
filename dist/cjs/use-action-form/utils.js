...
const useFindExistingRecord = (modelManager, findBy, options) => {
    ...
    if (typeof findBy === "string") {
        ...
        return (0, hooks_js_1.useFindOne)(modelManager, findBy, options);
    }
    ...
    if (isValidFindByObject) {
        ...
        // eslint-disable-next-line react-hooks/rules-of-hooks
        return hooks_js_1.useFindBy(finder, findByValue, options);
    }
    return noopUseFindExistingRecordResponse;
};
...
const disambiguateDefaultValues = (coreImplementation, data, initialData, action) => {
...
const reshapeDataForGraphqlApi = async (client, defaultValues, data) => {
    const referencedTypes = client[Symbol.for("gadget/modelRelationships")];
    if (!referencedTypes) {
...
const getNestedBelongsToGraphqlApiInput = (props) => {
    ...
    if (input[belongsToFieldName] && "__typename" in input[belongsToFieldName]) {
        const result = (0, utils_js_1.omit)(input[belongsToFieldName], ["_link"], true);
