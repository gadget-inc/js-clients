...
const hydrateRecord = (response, record, modelManager) => {
    const hydrator = (0, exports.getHydrator)(response);
    if (hydrator) {
        record = hydrator.apply(record);
    }
    return new GadgetRecord_js_1.GadgetRecord(record, modelManager);
};
exports.hydrateRecord = hydrateRecord;
const hydrateRecordArray = (response, records, modelManager) => {
    const hydrator = (0, exports.getHydrator)(response);
    if (hydrator) {
        records = hydrator.apply(records);
    }
    return records === null || records === void 0 ? void 0 : records.map((record) => new GadgetRecord_js_1.GadgetRecord(record, modelManager));
};
exports.hydrateRecordArray = hydrateRecordArray;
const hydrateConnection = (response, connection, modelManager) => {
    const nodes = connection.edges.map((edge) => edge.node);
    return (0, exports.hydrateRecordArray)(response, nodes, modelManager);
};
