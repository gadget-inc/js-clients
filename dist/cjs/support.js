...
exports.availableAuthenticationModes = [
    "apiKey",
    "browserSession",
    "anonymous",
    "internalAuthToken",
...
const _assertAllAuthKeysIncluded = true;
const maybeGetAuthenticationModeOptionsFromConnectionOptions = (options) => {
    const topLevelAuthModes = {};
    for (const key of exports.availableAuthenticationModes) {
        if (key in options) {
            topLevelAuthModes[key] = options[key];
        }
    }
    if ("authenticationMode" in options && Object.keys(topLevelAuthModes).length > 0) {
        throw new GadgetClientError("Declaring authentication modes at the top level and under the `authenticationMode` key at the same time is not allowed.");
    }
...
exports.maybeGetAuthenticationModeOptionsFromConnectionOptions = maybeGetAuthenticationModeOptionsFromConnectionOptions;
