...
const buildAutoFormFieldList = (fields, options) => {
    ...
    for (const fieldMeta of subset) {
        ...
        if (field.configuration.__typename === "GadgetHasManyThroughConfig") {
            ...
            const relatedJoinModelHasManyFieldMeta = joinModelHasManyFields.find(([_, f]) => f.configuration.__typename === "GadgetHasManyConfig" && joinModelApiIdentifier === f.configuration.relatedModel?.apiIdentifier);
            field.configuration.joinModelHasManyFieldApiIdentifier = relatedJoinModelHasManyFieldMeta?.[0];
        }
    }
