...
const useModelMetadata = (apiIdentifier, namespace) => {
    ...
    return {
        metadata: data
            ? (0, client_hooks_1.assert)(treeifyModelMetadata(data.gadgetMeta.modelAndRelatedModels), "no model metadata found in response from Gadget API")
            : data,
        fetching,
...
const useActionMetadata = (actionFunction) => {
    ...
    if (actionFunction.type === "globalAction") {
        ...
        const modelManager = (0, client_hooks_1.assert)((0, utils_js_1.getModelManager)(api, coreImplementation, actionFunction.modelApiIdentifier, actionFunction.namespace), "no model manager found for action function");
        let actionName;
    ...
    if (data && !error) {
        if ("globalAction" in data.gadgetMeta) {
            metadata = (0, client_hooks_1.assert)(data.gadgetMeta.globalAction, "no global action metadata found from Gadget API");
        }
        else {
            const typedData = data;
            (0, client_hooks_1.assert)(typedData.gadgetMeta.model?.action, "no model metadata found from Gadget API");
