...
const useActionForm = (action, options) => {
    ...
    }, [handleSubmit, formHook, handleSubmissionError, findResult.data, options, action.modelApiIdentifier, isForExistingRecord, runAction]);
    const proxiedFormState = new Proxy(formState, {
        get: (target, prop) => {
            if (prop === "isSubmitting") {
                ...
                return target.isSubmitSuccessful && !!actionResult.data && !actionResult.error;
            }
