...
const unionResultTypeToQueryCompilerFields = (unionResultType, selection) => {
    ...
    for (const [key, value] of Object.entries(unionResultType)) {
        output[key] = (0, tiny_graphql_query_compiler_1.On)(value.hasReturnType ? { result: true } : fieldSelectionToQueryCompilerFields(selection, false));
    }
    return output;
};
...
const actionResultFieldSelection = (modelSelectionField, selection, hasReturnType, unionResultType) => {
    return {
        ...
        [modelSelectionField]: selection && !hasReturnType
            ? unionResultType
...
const actionOperation = (operation, defaultSelection, modelApiIdentifier, modelSelectionField, variables, options, namespace, isBulkAction, hasReturnType, unionResultType) => {
    const selection = (options === null || options === void 0 ? void 0 : options.select) || defaultSelection;
    let fields = {
        [operation]: (0, tiny_graphql_query_compiler_1.Call)(variableOptionsToVariables(variables), actionResultFieldSelection(modelSelectionField, selection, hasReturnType, unionResultType)),
    };
    fields = (0, support_js_1.namespacify)(namespace, fields);
