...
const cO = new B({
  ...
  shift(O, e, a, r) {
    if (e == C) return new s(O, X(r, r.pos), $);
    if (e == z || e == Z) return new s(O, X(r, r.pos), m);
    if (e == c) return O.parent;
    if (e == tO || e == PO) return new s(O, 0, d);
    if (e == G && O.type == g) return O.parent;
    if (e == sO) {
      let o = /[1-9]/.exec(r.read(r.pos, a.pos));
      if (o) return new s(O, O.depth + +o[0], g);
    }
    return O;
  },
  hash(O) {
...
const fO = new S(
    (O, e) => {
      ...
      if ((P(a) || a < 0) && e.context.type != d) {
        ...
        (r < e.context.depth || (r == e.context.depth && e.context.type == $ && (O.next != 45 || !l(O.peek(1))))) &&
          O.next != -1 &&
          !P(O.next) &&
          O.next != 35 &&
          O.acceptToken(c, -r);
  ...
  XO = new S(
    (O, e) => {
      ...
      if (O.next == 45) O.advance(), l(O.next) && O.acceptToken(e.context.type == $ && e.context.depth == X(O, O.pos - 1) ? H : C);
      else if (O.next == 63) O.advance(), l(O.next) && O.acceptToken(e.context.type == m && e.context.depth == X(O, O.pos - 1) ? F : Z);
      else {
        let a = O.pos;
        for (;;)
          if (x(O.next)) {
            if (O.pos == a) return;
            O.advance();
          } else if (O.next == 33) _(O);
          else if (O.next == 38) p(O);
          else if (O.next == 42) {
            p(O);
            break;
        ...
        if (O.next == 58) {
          ...
          let r = O.peek(1);
          l(r) && O.acceptTokenTo(e.context.type == m && e.context.depth == X(O, a) ? K : z, a);
        }
      }
    },
    { contextual: !0 }
...
function p(O) {
  for (O.advance(); !l(O.next) && i(O.tag) != "f"; ) O.advance();
}
function T(O, e) {
  let a = O.next,
    r = !1,
...
const iO = "iiisiiissisfissssssssssssisssiiissssssssssssssssssssssssssfsfssissssssssssssssssssssssssssfif";
function i(O) {
  return O < 33 ? "u" : O > 125 ? "s" : iO[O - 33];
}
function k(O, e) {
  let a = i(O);
  return a != "u" && !(e && a == "f");
...
function w(O, e, a, r) {
  if (i(O.next) == "s" || ((O.next == 63 || O.next == 58 || O.next == 45) && k(O.peek(1), a))) O.advance();
  else return !1;
  let o = O.pos;
  for (;;) {
    let t = O.next,
...
const SO = new S((O, e) => {
    if (O.next == 33) _(O), O.acceptToken(rO);
    else if (O.next == 38 || O.next == 42) {
      let a = O.next == 38 ? eO : aO;
      p(O), O.acceptToken(a);
    } else
      O.next == 39 || O.next == 34 ? (T(O, !1), O.acceptToken(OO)) : w(O, !1, e.context.type == d, e.context.depth) && O.acceptToken(J);
  }),
  kO = new S((O, e) => {
    let a = e.context.type == g ? e.context.depth : -1,
      r = O.pos;
    O: for (;;) {
      let o = 0,
        t = O.next;
      for (; t == 32; ) t = O.peek(++o);
      if ((!o && (f(O, 45, o) || f(O, 46, o))) || (!P(t) && (a < 0 && (a = Math.max(e.context.depth + 1, o)), o < a))) break;
      for (;;) {
        if (O.next < 0) break O;
        let Q = P(O.next);
...
const xO = y.define({ name: "yaml-frontmatter", parser: gO.configure({ props: [W({ DashLine: n.meta })] }) });
function zO(O) {
...
export { ZO as yaml, zO as yamlFrontmatter, L as yamlLanguage };
