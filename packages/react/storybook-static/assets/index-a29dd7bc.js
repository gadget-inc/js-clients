...
function k(O) {
  let e;
  return O.next == o && ((e = O.peek(1)) == o || e == S);
}
const tO = new r(
    (O, e) => {
      if (e.dialectEnabled(c)) {
        let a;
        if (O.next < 0 && e.canShift(x)) O.acceptToken(x);
        else if (((a = O.peek(-1)) == l || a < 0) && e.canShift(Z)) {
          let Q = 0;
          for (; O.next != l && R.includes(O.next); ) O.advance(), Q++;
          O.next == l || k(O) ? O.acceptToken(Z, -Q) : Q && O.acceptToken(s);
        } else if (O.next == l) O.acceptToken(L, 1);
        ...
        let a = 0;
        for (; R.includes(O.next); ) O.advance(), a++;
        a && O.acceptToken(s);
      }
    },
    { contextual: !0 }
  ...
  nO = new r((O, e) => {
    if (k(O)) {
      if ((O.advance(), e.dialectEnabled(c))) {
        let a = -1;
        for (let Q = 1; ; Q++) {
          let t = O.peek(-Q - 1);
          if (t == l || t < 0) {
        ...
        if (a > -1) {
          ...
          for (O.advance(); O.next >= 0; )
            if (O.next == l) {
              O.advance();
              let n = 0;
              for (; O.next != l && R.includes(O.next); ) n++, O.advance();
              if (n < a) {
                t = -n - 1;
                break;
              }
            } else if (Q && O.next == S && O.peek(1) == o) {
              t = 2;
              break;
            } else O.advance();
          O.acceptToken(Q ? v : h, t);
          return;
        }
      }
      if (O.next == o) {
  ...
  lO = new r((O, e) => {
    (O.next == aO || O.next == W) && e.dialectEnabled(c) && O.acceptToken(O.next == W ? A : B, 1);
  }),
  RO = new r((O, e) => {
    if (!e.dialectEnabled(c)) return;
    let a = e.context.depth;
    if (O.next < 0 && a) {
      O.acceptToken(X);
      return;
    }
    if (O.peek(-1) == l) {
      let t = 0;
      for (; O.next != l && R.includes(O.next); ) O.advance(), t++;
      t != a && O.next != l && !k(O) && (t < a ? O.acceptToken(X, -t) : O.acceptToken(_));
    }
  }),
  rO = new r((O, e) => {
    for (let a = !1, Q = 0, t = 0; ; t++) {
      let { next: n } = O;
      if (d(n) || n == i || n == m || (a && q(n))) !a && (n != i || t > 0) && (a = !0), Q === t && n == i && Q++, O.advance();
      else if (n == y && O.peek(1) == T) {
        O.acceptToken(K, 2);
        break;
      } else {
        a && O.acceptToken(n == J ? F : Q == 2 && e.canShift(p) ? p : I);
        break;
      }
    }
  }),
  iO = new r((O) => {
    if (O.next == eO) {
      for (O.advance(); d(O.next) || O.next == i || O.next == m || q(O.next); ) O.advance();
      O.next == y && O.peek(1) == T ? O.acceptToken(D, 2) : O.acceptToken(N);
    }
  }),
  oO = new r((O) => {
    if (R.includes(O.peek(-1))) {
      let { next: e } = O;
      (d(e) || e == m || e == y || e == OO || e == H || e == M || e == i || e == QO) && O.acceptToken(C);
    }
  }),
  dO = new r((O) => {
    if (!R.includes(O.peek(-1))) {
      let { next: e } = O;
      if ((e == $O && (O.advance(), O.acceptToken(f)), d(e))) {
        do O.advance();
        while (d(O.next));
        O.acceptToken(f);
      }
...
const SO = new g(null, 0),
  cO = new z({
    start: SO,
    shift(O, e, a, Q) {
      return e == _ ? new g(O, a.pos - Q.pos) : e == X ? O.parent : O;
    },
    hash(O) {
      return O.hash;
    },
  ...
  sO = U({
    "AtKeyword import charset namespace keyframes media supports include mixin use forward extend at-root": $.definitionKeyword,
    ...
    SassVariableName: $.special($.variableName),
    Callee: $.operatorKeyword,
    Unit: $.unit,
    "UniversalSelector NestingSelector IndentedMixin IndentedInclude": $.definitionOperator,
    MatchOp: $.compareOperator,
    "ChildOp SiblingOp, LogicOp": $.logicOperator,
    BinOp: $.arithmeticOperator,
    "Important Global Default": $.modifier,
    Comment: $.blockComment,
    LineComment: $.lineComment,
    ColorLiteral: $.color,
    "ParenthesizedContent StringLiteral": $.string,
    "InterpolationStart InterpolationContinue InterpolationEnd": $.meta,
    ': "..."': $.punctuation,
    ...
    parser: yO.configure({
      props: [
        j.add({
          Block: u,
          Comment(O, e) {
            return { from: O.from + 2, to: e.sliceDoc(O.to - 2, O.to) == "*/" ? O.to - 2 : O.to };
          },
        }),
        Y.add({ Declaration: b() }),
      ],
    }),
    languageData: { commentTokens: { block: { open: "/*", close: "*/" }, line: "//" }, indentOnInput: /^\s*\}$/, wordChars: "$-" },
  }),
  kO = P.configure({
    dialect: "indented",
    props: [Y.add({ "Block RuleSet": (O) => O.baseIndent + O.unit }), j.add({ Block: (O) => ({ from: O.from, to: O.to }) })],
  }),
  fO = w((O) => O.name == "VariableName" || O.name == "SassVariableName");
function gO(O) {
  return new E(O != null && O.indented ? kO : P, P.data.of({ autocomplete: fO }));
}
export { gO as sass, fO as sassCompletionSource, P as sassLanguage };
