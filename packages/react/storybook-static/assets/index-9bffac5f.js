...
const _ = 177,
  ...
  aO = new s(
    (Q, e) => {
      for (
        let n = 0, a = Q.next;
        ((e.context && (a < 0 || a == M || a == H || (a == $ && Q.peek(n + 1) == $))) || a == QO || a == eO) && Q.acceptToken(_),
          !(a != K && a != OO);
...
const iO = new R({ start: !1, shift: (Q, e) => (e == q ? Q : tO.has(e)) }),
  XO = Y({
  ...
  oO = [
    ...
    X("func (${receiver}) ${name}(${params}) ${type} {\n	${}\n}", { label: "func", detail: "method declaration", type: "keyword" }),
...
function m(Q, e) {
  ...
  return (
    e.cursor(b.IncludeAnonymous).iterate((i) => {
      if (t) t = !1;
      else if (i.name) {
        let P = cO[i.name];
...
const $O = "interface struct chan map package go return break continue goto fallthrough else defer range true false nil".split(" ").map(lO);
function dO() {
  let Q = oO.concat($O);
  return new j(r, [r.data.of({ autocomplete: G(W, U(Q)) }), r.data.of({ autocomplete: rO })]);
}
export { dO as go, r as goLanguage, rO as localCompletionSource, oO as snippets };
