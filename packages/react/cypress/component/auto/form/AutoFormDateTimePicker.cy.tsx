...
describeForEachAutoAdapter(
  ...
  ({ name, adapter: { AutoForm, AutoDateTimePicker, AutoSubmit }, wrapper, clickOptions }) => {
    ...
    const getClearedPolarisWCDateInput = (fieldId: string) => {
      return cy.get(`[id="${fieldId}"]`).shadow().find("input").clear({ force: true });
    };
    ...
    describe("date only", () => {
      it(
        ...
        () => {
          ...
          if (name === SUITE_NAMES.POLARIS) {
            ...
            cy.get("#test-date").click(clickOptions);
      ...
      it("can change the date", () => {
        ...
        cy.get("#test-date").click(clickOptions);
        if (name === SUITE_NAMES.POLARIS) {
          cy.get(`[aria-label='Thursday March 4 2021']`).click(clickOptions);
        } else if (name === SUITE_NAMES.SHADCN) {
          cy.get(`[data-day="2021-03-04"]`).click(clickOptions);
        } else if (name === SUITE_NAMES.POLARIS_WC) {
          getClearedPolarisWCDateInput("test-date").type("2021-03-04", { force: true });
        }
      });
      ...
      it("can change the date across a DST boundary", () => {
        ...
        if (name === SUITE_NAMES.POLARIS) {
          cy.get(`[aria-label='Wednesday March 17 2021']`).click(clickOptions);
        } else if (name === SUITE_NAMES.SHADCN) {
          cy.get(`[data-day="2021-03-17"]`).click(clickOptions);
        } else if (name === SUITE_NAMES.POLARIS_WC) {
          getClearedPolarisWCDateInput("test-date").type("2021-03-17", { force: true });
        }
      });
    });
    describe("date and time", () => {
      ...
      it("can change the date", () => {
        ...
        if (name === SUITE_NAMES.SHADCN) {
          cy.get(`[aria-label='Thursday, March 4th, 2021']`).click(clickOptions);
        }
      ...
      it("can enter an invalid time and show an error", () => {
        ...
        if (name === SUITE_NAMES.POLARIS_WC) {
          // Polaris WC uses native time input which prevents invalid times
          return;
        }
        ...
        if (name === SUITE_NAMES.POLARIS) {
          cy.get("#test-time").click(clickOptions).clear().type("foo");
          cy.get("body").click(clickOptions);

          cy.contains("Invalid time format");
          cy.get("#test-time").click(clickOptions).clear();
      ...
      it("can show the selected date", () => {
        ...
        if (name === SUITE_NAMES.POLARIS_WC) {
          getClearedPolarisWCDateInput("test-date").type("2021-03-04", { force: true });
          cy.get("#test-date").should("have.value", "2021-03-04");
        }
      });
    });
  }
