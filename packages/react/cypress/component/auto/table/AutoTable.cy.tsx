...
describeForEachAutoAdapter("AutoTable", ({ name, adapter: { AutoTable }, wrapper }) => {
  ...
  it("can render for only some fields", () => {
    cy.mountWithWrapper(<AutoTable model={api.widget} select={{ id: true, name: false, inventoryCount: true }} />, wrapper);
    cy.contains("1");
    cy.contains("2");
    cy.contains("3");
  });

  it("can render for an explicit selection which excludes the id", () => {
    cy.mountWithWrapper(<AutoTable model={api.widget} select={{ name: true, inventoryCount: false }} />, wrapper);
    cy.contains("foo");
    cy.contains("bar");
    cy.contains("baz");
  ...
  it("can render for a large number of rows", () => {
    cy.mountWithWrapper(<AutoTable model={api.gizmo} />, wrapper);
    cy.contains("gizmo 10");
  });

  it("can paginate forwards and backwards", () => {
    cy.mountWithWrapper(<AutoTable model={api.gizmo} pageSize={50} />, wrapper);
  ...
  it("can start listing data at a certain page", async () => {
    ...
    cy.mountWithWrapper(<AutoTable model={api.gizmo} pageSize={50} initialCursor={firstPage.endCursor} />, wrapper);
    cy.contains("gizmo 50");
    cy.contains("gizmo 99");
    cy.get(nextPageSelector!).click();
    cy.contains("gizmo 100");
  ...
  it("renders an empty table if the backend returns no rows", () => {
    cy.intercept("POST", "https://js-clients-test--development.gadget.app/api/graphql?operation=gizmos", {
      data: { gizmos: { edges: [], pageInfo: { hasNextPage: false, hasPreviousPage: false } } },
    });
