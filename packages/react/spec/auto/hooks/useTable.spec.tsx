...
describe("useTable hook", () => {
  ...
  describe("custom cell renderer", () => {
    ...
    it("should not be sortable when no sortByField is provided", async () => {
      const result = getUseTableResult({
        columns: [
          "name",
          {
      ...
      expect(customColumn?.type === "CustomRenderer" && customColumn?.sortByField).toBeUndefined();
    });

    it("should be sortable when a sortable field is provided via sortByField", async () => {
      const result = getUseTableResult({
        columns: [
          {
            header: "Custom Name",
      ...
      expect(customColumn?.type === "CustomRenderer" && customColumn?.sortByField).toBe("name");
    });

    it("should not be sortable when sortByField is provided but sortable is set to false", async () => {
      const result = getUseTableResult({
        columns: [
          {
            header: "Custom Name",
    ...
    it("should not be sortable when the field itself is not sortable", async () => {
      ...
      const customColumn = result.current[0].columns?.find((col) => col.type === "CustomRenderer");
      expect(customColumn?.sortable).toBe(false);
      expect(customColumn?.type === "CustomRenderer" && customColumn?.sortByField).toBe("id");
    });
    ...
    it("should throw an error when sortByField references a non-existent field", async () => {
      let error: Error | undefined;
      try {
        getUseTableResult({
          columns: [
            {
              header: "Custom Invalid",
              render: ({ record }) => <div>Custom</div>,
              sortByField: "nonExistentField",
            },
          ],
        });
        await loadMockWidgetModelMetadata();
      ...
      expect(error!.message).toBe(
        "Field 'nonExistentField' specified as sortByField on custom column 'Custom Invalid' does not exist in the model"
      );
    });

    it("should throw an error when sortByField is a relationship path", async () => {
      let error: Error | undefined;
      try {
        getUseTableResult({
          columns: [
            {
              header: "Custom Related",
              render: ({ record }) => <div>Custom</div>,
              sortByField: "hasMany.edges.node.someField",
            },
          ],
        });
        await loadMockWidgetModelMetadataForRelationship();
      ...
      expect(error!.message).toBe(
        "Field path 'hasMany.edges.node.someField' on custom column 'Custom Related' cannot be a relationship path. Sorting through relationships is not supported."
      );
    });
  });
