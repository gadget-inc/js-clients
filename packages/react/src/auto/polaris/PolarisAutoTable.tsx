...
import type { FindManyFunction, GadgetRecord, SortOrder } from "@gadgetinc/core";
...
const maybeGetColumnIndex = (columns: TableColumn[], apiIdentifier: string | undefined) => {
  return columns.findIndex((column) => {
    if (column.type === "CustomRenderer") {
      return column.sortByField === apiIdentifier;
    }
    return column.field === apiIdentifier;
  });
};
...
>(
  ...
  const handleColumnSort = (headingIndex: number) => {
    if (columns) {
      ...
      const columnApiIdentifier = currentColumn.type === "CustomRenderer" ? currentColumn.sortByField : currentColumn.field;
