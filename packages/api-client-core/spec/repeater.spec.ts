...
describe("Repeater", () => {
  ...
  test("throw error in executor after async push and stop", async () => {
    const error = new Error("throw error in executor after async push and stop");
    const r = new Repeater(async (push, stop) => {
      await push(1);
      await push(2);
      await push(3);
  ...
  test("throw error after stopping with error and pushing rejection", async () => {
    const error1 = new Error("throw error after stopping with error and pushing rejection 1");
    const error2 = new Error("throw error after stopping with error and pushing rejection 2");
    const error3 = new Error("throw error after stopping with error and pushing rejection 3");
    const r = new Repeater((push, stop) => {
      push(1);
      push(2);
      push(Promise.reject(error1));
  ...
  test("return rejection from executor after async pushes", async () => {
    const error = new Error("return rejection from executor after async pushes");
    const r = new Repeater(async (push) => {
      await push(1);
      await push(2);
  ...
  test("pushes resolve to value passed to next", async () => {
    ...
    await expect(Promise.race([push4, delayPromise(100, -1000)])).resolves.toEqual(-1000);
  });
  ...
  test("pushes resolve to value passed to next irregular", async () => {
    ...
    await expect(push2).resolves.toEqual(-3);
    await expect(push3).resolves.toEqual(-4);
    await expect(Promise.race([push4, delayPromise(1, -1000)])).resolves.toEqual(-1000);
  });
  ...
  test("results settle in order with return method and rejection", async () => {
    const error = new Error("results settle in order with return method and rejection");
    const r = new Repeater((push) => {
      push(delayPromise(100, 1));
      push(delayPromise(10, 2));
      push(Promise.reject(error));
